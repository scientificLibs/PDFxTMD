//
// Copyright (c) 2018-2019 Kris Jusiak (kris at jusiak dot
// net)
//
...
#pragma once
    ...
    !defined(__cpp_decltype) || !defined(__cpp_alias_templates) ||                                 \
    !defined(__cpp_generic_lambdas) || !defined(__cpp_constexpr) ||                                \
    !defined(__cpp_return_type_deduction) || !defined(__cpp_fold_expressions) ||                   \
    !defined(__cpp_static_assert) || !defined(__cpp_delegating_constructors)
...
template <class, std::size_t> struct mappings final
{
...
template <std::size_t, class...> constexpr std::size_t mappings_size_impl(...)
{
    return {};
}

template <std::size_t N, class T, class... Ts>
constexpr auto mappings_size_impl(bool dummy) -> decltype(get(mappings<T, N>{}), std::size_t{})
{
    return 1 + mappings_size_impl<N + 1, T, Ts...>(dummy);
}

template <class... Ts> constexpr auto mappings_size()
{
    return mappings_size_impl<1, Ts...>(bool{});
}

template <class T, class = decltype(sizeof(T))> std::true_type is_complete_impl(bool);
template <class> std::false_type is_complete_impl(...);
template <class T> struct is_complete : decltype(is_complete_impl<T>(bool{}))
{
};

template <class T>
constexpr auto requires__(bool) -> decltype(std::declval<T>().template requires__<T>());
template <class> constexpr auto requires__(...) -> void;

template <class TExpr> class expr_wrapper final
{
...
struct non_owning_storage
{
    ...
    constexpr explicit non_owning_storage(T &&t) noexcept : ptr{&t}
    {
    }
...
struct dynamic_storage
{
    ...
    constexpr explicit dynamic_storage(T &&t) noexcept(std::is_nothrow_constructible_v<T_, T &&>)
        : ptr{new T_{std::forward<T>(t)}}, del{[](void *self) {
          ...
          copy{[](const void *other) -> void * {
              if constexpr (std::is_copy_constructible_v<T_>)
                  ...
                  throw std::runtime_error("dynamic_storage : erased type is not copy "
                                           "constructible");
          }}
    {
    }
    ...
    constexpr dynamic_storage(const dynamic_storage &other)
        : ptr{other.ptr ? other.copy(other.ptr) : nullptr}, del{other.del}, copy{other.copy}
    {
    }
    ...
    constexpr dynamic_storage(dynamic_storage &&other) noexcept
        : ptr{detail::exchange(other.ptr, nullptr)}, del{detail::exchange(other.del, nullptr)},
...
template <std::size_t Size, std::size_t Alignment = 8> struct local_storage
{
    ...
    constexpr explicit local_storage(T &&t) noexcept(std::is_nothrow_constructible_v<T_, T &&>)
        : ptr{new (&data) T_{std::forward<T>(t)}}, del{[](mem_t &self) {
          ...
          copy{[](mem_t &self, const void *other) -> void * {
              if constexpr (std::is_copy_constructible_v<T_>)
                  ...
                  throw std::runtime_error("local_storage : erased type is not copy "
                                           "constructible");
          }},
          move{[](mem_t &self, void *other) -> void * {
              if constexpr (std::is_move_constructible_v<T_>)
                  ...
                  throw std::runtime_error("local_storage : erased type is not move "
                                           "constructible");
          }}
    {
    ...
    constexpr local_storage(const local_storage &other)
        : ptr{other.ptr ? other.copy(data, other.ptr) : nullptr}, del{other.del}, copy{other.copy},
          move{other.move}
    {
    }
    ...
    constexpr local_storage(local_storage &&other)
        : ptr{other.ptr ? other.move(data, other.ptr) : nullptr}, del{other.del}, copy{other.copy},
          move{other.move}
    {
    }
...
template <std::size_t Size, std::size_t Alignment = 8> struct sbo_storage
{
    template <typename T_>
    struct type_fits
        : std::integral_constant<bool, sizeof(T_) <= Size && Alignment % alignof(T_) == 0>
    {
    };
    ...
    constexpr explicit sbo_storage(T &&t) noexcept(std::is_nothrow_constructible_v<T_, T &&>)
        : ptr{new (&data) T_{std::forward<T>(t)}}, del{[](mem_t &self_mem, void *) {
          ...
          copy{[](mem_t &self_mem, const void *other) -> void * {
              if constexpr (std::is_copy_constructible_v<T_>)
                  ...
                  throw std::runtime_error("sbo_storage : erased type is not copy "
                                           "constructible");
          }},
          move{[](mem_t &self_mem, void *&other) -> void * {
              if constexpr (std::is_move_constructible_v<T_>)
                  ...
                  throw std::runtime_error("sbo_storage : erased type is not move "
                                           "constructible");
          }}
    {
    }
    ...
    template <class T, class T_ = std::decay_t<T>,
              ...
              std::enable_if_t<!type_fits<T_>::value, bool> = true>
    constexpr explicit sbo_storage(T &&t) noexcept(std::is_nothrow_constructible_v<T_, T &&>)
        : ptr{new T_{std::forward<T>(t)}}, del{[](mem_t &, void *self_ptr) {
          ...
          move{[](mem_t &, void *&other) { return detail::exchange(other, nullptr); }}
    {
    }
...
class static_vtable
{
  ...
  public:
    ...
    explicit static_vtable(T &&, ptr_t *&vtable, std::integral_constant<std::size_t, Size>) noexcept
    {
...
class poly : detail::poly_base,
             public std::conditional_t<detail::is_complete<I>{}, I, detail::type_list<I>>
{
  public:
    template <class T, class T_ = std::decay_t<T>,
              std::enable_if_t<!std::is_same_v<T_, poly>,
                               bool> = true>
    ...
    constexpr poly &operator=(poly const &) noexcept(
        std::is_nothrow_copy_constructible_v<TStorage>) = default;
    constexpr poly(poly &&) noexcept(std::is_nothrow_move_constructible_v<TStorage>) = default;
    constexpr poly &operator=(poly &&) noexcept(std::is_nothrow_move_constructible_v<TStorage>) =
        default;

  private:
    template <class T, class T_ = std::decay_t<T>, class TRequires>
    constexpr explicit poly(T &&t,
                            const TRequires) noexcept(std::is_nothrow_constructible_v<T_, T &&>)
          ...
          
    {
        ...
        static_assert(std::is_copy_constructible_v<T_> || std::is_move_constructible_v<T_>,
...
constexpr auto call_impl(const poly_base &self, std::integral_constant<std::size_t, N>,
                         type_list<R>, const TExpr, Ts &&...args)
{
    void(typename mappings<I, N>::template set<type_list<TExpr, Ts...>>{});
    return reinterpret_cast<R (*)(void *, Ts...)>(self.vptr[N - 1])(self.ptr(),
                                                                    std::forward<Ts>(args)...);
}
...
template <class I, class T, std::size_t... Ns>
constexpr auto requires_impl(std::index_sequence<Ns...>)
    -> type_list<decltype(requires_impl<I>(decltype(get(mappings<T, Ns + 1>{})){}))...>;
} // namespace detail
...
template <class I, class T> constexpr auto extends(const T &) noexcept
{
...
#define REQUIRES(R, name, ...)                                                                     \
    ...
    {                                                                                              \
        return ::te::call<R>(                                                                      \
            [](auto &&self,                                                                        \
               auto &&...args) -> decltype(self.name(std::forward<decltype(args)>(args)...)) {     \
