...
class ITMD
{
  public:
    ...
    explicit ITMD(TMDApproachT tmdApproach)
        : pimpl_(new OwningModel<TMDApproachT>(std::move(tmdApproach)),
          ...
          tmdOperation1_(
              [](void *tmdfApproachBytes, double x, double kt2, double mu2, std::array<double, DEFAULT_TOTAL_PDFS>& output) -> void {
                  using Model = OwningModel<TMDApproachT>;
                  auto *const model = static_cast<Model *>(tmdfApproachBytes);
  ...
  private:
    ...
    using TMDOperation1 = void(void *, double, double, double, std::array<double, DEFAULT_TOTAL_PDFS>& output);
