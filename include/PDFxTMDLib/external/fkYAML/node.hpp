...
#if defined(FK_YAML_MAJOR_VERSION) && defined(FK_YAML_MINOR_VERSION) &&                            \
    defined(FK_YAML_PATCH_VERSION)
...
#define FK_YAML_NAMESPACE_VERSION                                                                  \
    FK_YAML_NAMESPACE_VERSION_CONCAT(FK_YAML_MAJOR_VERSION, FK_YAML_MINOR_VERSION,                 \
                                     FK_YAML_PATCH_VERSION)
...
// See https://devblogs.microsoft.com/cppblog/msvc-now-correctly-reports-__cplusplus/ for more
// details.
...
#if !defined(FK_YAML_HAS_CXX_23) && !defined(FK_YAML_HAS_CXX_20) &&                                \
    !defined(FK_YAML_HAS_CXX_17) && !defined(FK_YAML_HAS_CXX_14) && !defined(FK_YAML_CXX_11)
...
// switch usages of the std::to_chars()/std::from_chars() functions which have been available since
// C++17.
...
template <typename IntSeq0, typename IntSeq1> struct merger;
...
template <typename T, std::size_t Num> struct generator
{
    using type = typename merger<typename generator<T, Num / 2>::type,
                                 typename generator<T, Num - Num / 2>::type>::type;
};

template <typename T> struct generator<T, 0>
{
    using type = integer_sequence<T>;
};

template <typename T> struct generator<T, 1>
{
...
template <std::size_t Num> using make_index_sequence = make_integer_sequence<std::size_t, Num>;
...
/// @tparam Val
template <bool Val> using bool_constant = std::integral_constant<bool, Val>;
...
template <typename Trait> struct conjunction<Trait> : Trait
{
};
...
/// @tparam Rest The rest of traits passed as another conjunction template arguments if First::value
/// is true.
...
/// @tparam Trait Type trait to be checked if its ::value is true.
template <typename Trait> struct disjunction<Trait> : Trait
{
};
...
struct disjunction<First, Rest...>
    : std::conditional<First::value, First, disjunction<Rest...>>::type
{
};
...
template <typename Trait> struct negation : std::integral_constant<bool, !Trait::value>
{
};
...
/// @brief A wrapper function to call std::unreachable() (since C++23) or similar compiler specific
/// extensions.
/// @note This function is implemented only for better code optimization against dead code and thus
...
/// @tparam T A target type.
template <typename T> using container_type_t = typename T::container_type;
...
/// @tparam T A target type.
template <typename T> using has_iterator = is_detected<iterator_t, T>;

/// @brief Type traits to check if T has `key_type` member type.
/// @tparam T A target type.
template <typename T> using has_key_type = is_detected<key_type_t, T>;

/// @brief Type traits to check if T has `mapped_type` member type.
/// @tparam T A target type.
template <typename T> using has_mapped_type = is_detected<mapped_type_t, T>;

/// @brief Type traits to check if T has `value_type` member type.
/// @tparam T A target type.
template <typename T> using has_value_type = is_detected<value_type_t, T>;

/// @brief Type traits to check if T is a std::iterator_traits like type.
/// @tparam T A target type.
template <typename T>
struct is_iterator_traits
    : conjunction<is_detected<difference_type_t, T>, has_value_type<T>, is_detected<pointer_t, T>,
...
/// @tparam T A target type.
template <typename T> using has_container_type = is_detected<container_type_t, T>;

/// @brief Type traits to check if T has emplace member function.
/// @tparam T A target type.
template <typename T, typename... Args> using has_emplace = is_detected<emplace_fn_t, T, Args...>;

/// @brief Type traits to check if T has reserve member function.
/// @tparam T A target type.
template <typename T> using has_reserve = is_detected<reserve_fn_t, T>;
...
/// @tparam T A target type.
template <typename T>
struct has_begin_end<T, void_t<decltype(begin(std::declval<T>()), end(std::declval<T>()))>>
    : std::true_type
{
};
...
struct is_comparable<
    Comparator, T, U,
    void_t<decltype(std::declval<Comparator>()(std::declval<T>(), std::declval<U>())),
           decltype(std::declval<Comparator>()(std::declval<U>(), std::declval<T>()))>>
    : std::true_type
{
};
...
using is_usable_as_key_type =
    typename std::conditional<is_comparable<Comparator, ObjectKeyType, KeyType>::value,
                              std::true_type, std::false_type>::type;

/// @brief Type trait to check if IntegralType is of non-boolean integral types.
/// @tparam IntegralType A type to be checked.
/// @tparam typename N/A
template <typename IntegralType, typename = void> struct is_non_bool_integral : std::false_type
{
};

/// @brief A partial specialization of is_non_bool_integral if IntegralType is of non-boolean
/// integral types.
/// @tparam IntegralType A type to be checked.
template <typename IntegralType>
struct is_non_bool_integral<
    IntegralType, enable_if_t<conjunction<std::is_integral<IntegralType>,
                                          negation<std::is_same<bool, IntegralType>>>::value>>
    : std::true_type
{
};
...
template <typename... Types> using is_all_signed = conjunction<std::is_signed<Types>...>;
...
template <typename... Types> using is_all_unsigned = conjunction<std::is_unsigned<Types>...>;

/// @brief Type trait implementation to check if TargetIntegerType and CompatibleIntegerType are
...
/// @brief A partial specialization of is_compatible_integer_type_impl if TargetIntegerType and
/// CompatibleIntegerType are compatible integer types.
/// @tparam TargetIntegerType A target integer type.
/// @tparam CompatibleIntegerType A compatible integer type.
template <typename TargetIntegerType, typename CompatibleIntegerType>
struct is_compatible_integer_type_impl<
    TargetIntegerType, CompatibleIntegerType,
    enable_if_t<is_non_bool_integral<CompatibleIntegerType>::value>> : std::true_type
{
};

/// @brief Type traits to check if TargetIntegerType and CompatibleIntegerType are compatible
/// integer types.
/// @tparam TargetIntegerType A target integer type.
/// @tparam CompatibleIntegerType A compatible integer type.
template <typename TargetIntegerType, typename CompatibleIntegerType>
struct is_compatible_integer_type
    : is_compatible_integer_type_impl<TargetIntegerType, CompatibleIntegerType>
{
};
...
using is_iterator_of =
    std::is_same<remove_cv_t<typename std::iterator_traits<ItrType>::value_type>, T>;
...
/// @note This is required if inline variables are not available. C++11-14 do not provide such a
/// feature yet.
/// @tparam T A target type for the resulting static constant instance.
...
template <typename... Types> struct get_head_type;

/// @brief A specialization of get_head_type if variadic template has no arguments.
/// @tparam  N/A
template <> struct get_head_type<>
{
    /// @brief A head type
    using type = void;
};

/// @brief A partial specialization of get_head_type if variadic template has one or more
/// argument(s).
/// @tparam First The first type in the arguments
/// @tparam Rest The rest of the types in the arguments.
template <typename First, typename... Rest> struct get_head_type<First, Rest...>
{
    /// @brief A head type.
    using type = First;
};
...
/// @tparam Types Types of variadic template arguments.
template <typename... Types> using head_type = typename get_head_type<Types...>::type;
...
template <typename ValueType, typename = void> class node_value_converter;

/// @brief A class to store value of YAML nodes.
/// @sa https://fktn-k.github.io/fkYAML/api/basic_node/
template <template <typename, typename...> class SequenceType = std::vector,
          template <typename, typename, typename...> class MappingType = std::map,
          typename BooleanType = bool, typename IntegerType = std::int64_t,
...
template <typename T> struct is_basic_node : std::false_type
{
};
...
template <template <typename, typename...> class SequenceType,
          template <typename, typename, typename...> class MappingType, typename BooleanType,
          typename IntegerType, typename FloatNumberType, typename StringType,
          template <typename, typename> class Converter>
struct is_basic_node<basic_node<SequenceType, MappingType, BooleanType, IntegerType,
                                FloatNumberType, StringType, Converter>> : std::true_type
{
};
...
template <typename> class node_ref_storage;

/// @brief A struct to check the template parameter class is a kind of node_ref_storage_template
/// class.
/// @tparam T A type to be checked if it's a kind of node_ref_storage template class.
template <typename T> struct is_node_ref_storage : std::false_type
{
};
...
template <typename T> struct is_node_ref_storage<node_ref_storage<T>> : std::true_type
{
};
...
/// @brief Type traits to check if T is a compatible type for BasicNodeType in terms of from_node
/// function.
...
/// @brief A partial specialization of has_from_node if T is not a basic_node template instance
/// type.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam T A target type passed to from_node function.
...
/// @brief Type traits to check if T is a compatible type for BasicNodeType in terms of to_node
/// function.
...
/// @tparam typename N/A
template <typename BasicNodeType, typename T, typename = void> struct has_to_node : std::false_type
{
};
...
struct has_to_node<BasicNodeType, T, enable_if_t<!is_basic_node<T>::value>>
{
    ...
    // NOLINTNEXTLINE(readability-identifier-naming)
    static constexpr bool value =
        is_detected_exact<void, to_node_function_t, converter, BasicNodeType &, T>::value;
};
...
/// @brief Type traits implementation of is_node_compatible_type to check if CompatibleType is a
/// compatible type for BasicNodeType.
/// @tparam BasicNodeType A basic_node template instance type.
...
/// @brief A partial specialization of is_node_compatible_type_impl if CompatibleType is a complete
/// type and is compatible for BasicNodeType.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam CompatibleType A target type for compatibility check.
template <typename BasicNodeType, typename CompatibleType>
struct is_node_compatible_type_impl<
    BasicNodeType, CompatibleType,
    enable_if_t<conjunction<is_complete_type<CompatibleType>,
                            has_to_node<BasicNodeType, CompatibleType>>::value>> : std::true_type
{
};
...
class uri_encoding
{
  ...
  private:
    ...
    {
        // Check if the current character is one of reserved/unreserved characters which are allowed
        // for use. See the following links for details:
...
class exception : public std::exception
{
  ...
  public:
    /// @brief Returns an error message internally held. If nothing, a non-null, empty string will
    /// be returned.
...
class invalid_encoding : public exception
{
  ...
  private:
    static std::string generate_error_message(const char *msg,
                                              const std::initializer_list<uint8_t> &u8) noexcept
    {
    ...
    static std::string generate_error_message(const char *msg, std::array<char16_t, 2> u16) noexcept
    {
        ...
        return detail::format("invalid_encoding: %s in=[ 0x%04x, 0x%04x ]", msg,
                              static_cast<uint16_t>(u16[0]), static_cast<uint16_t>(u16[1]));
    }
...
{
  public:
    /// @brief Constructs a new parse_error object with an error message and counts of lines and
    /// colums at the error.
    /// @param[in] msg An error message.
  ...
  private:
    static std::string generate_error_message(const char *msg, uint32_t lines,
                                              uint32_t cols_in_line) noexcept
    {
...
class out_of_range : public exception
{
  public:
    ...
    explicit out_of_range(int index) noexcept : exception(generate_error_message(index).c_str())
    {
    }

    /// @brief Construct a new out_of_range object with invalid key contents.
    /// @param[in] key Invalid key contents
    explicit out_of_range(const char *key) noexcept : exception(generate_error_message(key).c_str())
    {
    }
...
inline void from_utf16(std::array<char16_t, 2> utf16, std::array<uint8_t, 4> &utf8,
                       uint32_t &consumed_size, uint32_t &encoded_size)
{
    if (utf16[0] < 0x80u)
    {
    ...
    else if (utf16[0] <= 0x7FFu)
    {
        const auto utf8_chunk =
            static_cast<uint16_t>(0xC080u | ((utf16[0] & 0x07C0u) << 2) | (utf16[0] & 0x3Fu));
    ...
    {
        const auto utf8_chunk =
            static_cast<uint32_t>(0xE08080u | ((utf16[0] & 0xF000u) << 4) |
                                  ((utf16[0] & 0x0FC0u) << 2) | (utf16[0] & 0x3Fu));
...
inline void from_utf32(const char32_t utf32, std::array<uint8_t, 4> &utf8, uint32_t &encoded_size)
{
    ...
    {
        const auto utf8_chunk =
            static_cast<uint16_t>(0xC080u | ((utf32 & 0x07C0u) << 2) | (utf32 & 0x3Fu));
        utf8[0] = static_cast<uint8_t>((utf8_chunk & 0xFF00u) >> 8);
        utf8[1] = static_cast<uint8_t>(utf8_chunk & 0x00FFu);
        encoded_size = 2;
    }
    ...
    {
        const auto utf8_chunk = static_cast<uint32_t>(0xE08080u | ((utf32 & 0xF000u) << 4) |
                                                      ((utf32 & 0x0FC0u) << 2) | (utf32 & 0x3F));
        utf8[0] = static_cast<uint8_t>((utf8_chunk & 0xFF0000u) >> 16);
        utf8[1] = static_cast<uint8_t>((utf8_chunk & 0x00FF00u) >> 8);
        utf8[2] = static_cast<uint8_t>(utf8_chunk & 0x0000FFu);
        encoded_size = 3;
    ...
    {
        const auto utf8_chunk = static_cast<uint32_t>(0xF0808080u | ((utf32 & 0x1C0000u) << 6) |
                                                      ((utf32 & 0x03F000u) << 4) |
...
enum class chomping_indicator_t : std::uint8_t
{
    ...
    CLIP,  //!< preserves final line breaks but excludes trailing empty lines. no indicator means
           //!< this type.
...
/// @note
/// This class is a minimal implementation of std::basic_string_view which has been available since
/// C++17 but pretty useful and efficient for referencing/investigating character sequences.
/// @warning
/// This class intentionally omits a lot of value checks to improve efficiency. Necessary checks
/// should be made before calling this class' APIs for safety.
/// @tparam CharT Character type
/// @tparam Traits Character traits type which defaults to std::char_traits<CharT>.
template <typename CharT, typename Traits = std::char_traits<CharT>> class basic_str_view
{
    ...
    static_assert(std::is_same<CharT, typename Traits::char_type>::value,
                  "CharT & Traits::char_type must be the same type.");
  ...
  public:
    ...
    /// @param p_str A pointer to a character sequence. (Must be null-terminated, or an undefined
    /// behavior.)
    template <
        typename CharPtrT,
        enable_if_t<conjunction<negation<std::is_array<CharPtrT>>, std::is_pointer<CharPtrT>,
                                disjunction<std::is_same<CharPtrT, value_type *>,
                                            std::is_same<CharPtrT, const value_type *>>>::value,
                    int> = 0>
    FK_YAML_CXX17_CONSTEXPR basic_str_view(CharPtrT p_str) noexcept
        : m_len(traits_type::length(p_str)), mp_str(p_str)
    {
    }

    /// @brief Constructs a basic_str_view object from a C-style char array.
    /// @note
    /// This constructor assumes the last element is the null character ('\0'). If that's not
    /// desirable, consider using one of the other overloads.
    /// @tparam N The size of a C-style char array.
    /// @param str A C-style char array. (Must be null-terminated)
    template <std::size_t N>
    constexpr basic_str_view(const value_type (&str)[N]) noexcept : m_len(N - 1), mp_str(&str[0])
    {
    }
    ...
    /// @brief Constructs a basic_str_view object from a pointer to a character sequence and its
    /// size.
    ...
    template <typename ItrType,
              enable_if_t<conjunction<is_iterator_of<ItrType, CharT>,
                                      std::is_base_of<std::random_access_iterator_tag,
                                                      typename std::iterator_traits<
                                                          ItrType>::iterator_category>>::value,
                          int> = 0>
    basic_str_view(ItrType first, ItrType last) noexcept : m_len(last - first), mp_str(&*first)
    {
    }
    ...
    basic_str_view(const std::basic_string<CharT> &str) noexcept
        : m_len(str.length()), mp_str(str.data())
    {
    }
    ...
    /// @warning Throws an fkyaml::out_of_range exception if the position exceeds the character
    /// sequence size.
    /// @param pos The position of the target element.
    /// @return The element at the given position.
    ...
    /// @warning Throws an fkyaml::out_of_range exception if the given `pos` is bigger than the
    /// length.
    ...
    /// @brief Compares the referenced character sequence values with the given basic_str_view
    /// object.
    ...
    int compare(basic_str_view sv) const noexcept
    {
        ...
        if (ret == 0)
        {
            ...
            const difference_type diff = m_len > sv.m_len
                                             ? m_len - sv.m_len
                                             : static_cast<difference_type>(-1) *
                                                   static_cast<difference_type>(sv.m_len - m_len);
    ...
    /// @brief Compares the referenced character sequence values from `pos1` by `n1` characters with
    /// `sv`.
    ...
    /// @brief Compares the referenced character sequence value from `pos1` by `n1` characters with
    /// `sv` from `pos2` by `n2` characters.
    ...
    /// @brief Compares the referenced character sequence from `pos1` by `n1` characters with `s`
    /// character sequence.
    /// @param pos1 The offset of the beginning element in this character sequence.
    ...
    /// @brief Compares the referenced character sequence from `pos1` by `n1` characters with `s`
    /// character sequence by `n2` characters.
    /// @param pos1 The offset of the beginning element in this character sequence.
    /// @param n1 The length of this character sequence used fo comparison.
    /// @param s The pointer to a character sequence to compare with.
    ...
    /// @brief Finds the beginning position of `sv` characters in this referenced character
    /// sequence.
    /// @param sv The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the beginning position of `s` character sequence by `n` characters in this
    ...
    /// @brief Finds the beginning position of `s` character sequence in this referenced character
    /// sequence.
    /// @param s The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Retrospectively finds the beginning position of `sv` characters in this referenced
    /// character sequence.
    /// @param sv The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Retrospectively finds the beginning position of `c` character in this referenced
    /// character sequence.
    /// @param sv The character to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Retrospectively finds the beginning position of `s` character sequence by `n`
    /// characters in this referenced character sequence.
    /// @param s The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    /// @param n The length of `s` character sequence used for comparison.
    ...
    /// @brief Retrospectively finds the beginning position of `s` character sequence in this
    /// referenced character sequence.
    /// @param s The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the first occurence of `sv` character sequence in this referenced character
    /// sequence.
    /// @param sv The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the first occurence of `s` character sequence by `n` characters in this
    /// referenced character sequence.
    /// @param s The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the first occurence of `s` character sequence in this referenced character
    /// sequence.
    /// @param s The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the last occurence of `sv` character sequence in this referenced character
    /// sequence.
    /// @param sv The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the last occurence of `s` character sequence by `n` characters in this
    /// referenced character sequence.
    /// @param s The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the last occurence of `s` character sequence in this referenced character
    /// sequence.
    /// @param s The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the first absence of `sv` character sequence in this referenced character
    /// sequence.
    /// @param sv The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the first absence of `s` character sequence by `n` characters in this
    /// referenced character sequence.
    /// @param s The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the first absence of `s` character sequence in this referenced character
    /// sequence.
    /// @param s The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the last absence of `sv` character sequence in this referenced character
    /// sequence.
    /// @param sv The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the last absence of `s` character sequence by `n` characters in this referenced
    /// character sequence.
    /// @param s The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
    ...
    /// @brief Finds the last absence of `s` character sequence in this referenced character
    /// sequence.
    /// @param s The character sequence to compare with.
    /// @param pos The offset of the search beginning position in this referenced character
    /// sequence.
...
template <typename CharT, typename Traits>
inline bool operator==(basic_str_view<CharT, Traits> lhs,
                       basic_str_view<CharT, Traits> rhs) noexcept
{
...
/// @return true if the two objects are the same, false otherwise.
template <typename CharT, typename Traits>
inline bool operator==(const std::basic_string<CharT, Traits> &lhs,
                       basic_str_view<CharT, Traits> rhs) noexcept
{
...
class position_tracker
{
  public:
    ...
    /// @note This function doesn't support cases where cur_pos has moved backward from the last
    /// call.
...
struct lexical_token
{
    ...
    lexical_token(lexical_token_t t, str_view s) noexcept : type(t), str(s)
    {
    }

    lexical_token(lexical_token_t t) noexcept : type(t)
    {
    }
...
class lexical_analyzer
{
  ...
  public:
    ...
    explicit lexical_analyzer(str_view input_buffer) noexcept
        : m_input_buffer(input_buffer), m_cur_itr(m_input_buffer.begin()),
    ...
    lexical_token get_next_token()
    {
        ...
        case '`':
            emit_error("Any token cannot start with grave accent(`). It is a reserved indicator "
                       "for YAML.");
        case '\"':
            ++m_token_begin_itr;
        ...
        case '>': {
            ...
            const lexical_token_t type = *m_token_begin_itr == '|'
                                             ? lexical_token_t::BLOCK_LITERAL_SCALAR
            ...
            return {type,
                    determine_block_scalar_content_range(base_indent, m_block_scalar_header.indent,
  ...
  private:
    uint32_t get_current_indent_level(const char *p_line_end)
    {
        ...
        std::size_t line_begin_pos =
            str_view(m_input_buffer.begin(), p_line_end - 1).find_last_of('\n');
        ...
        if (context > 0)
        {
            ...
            // In any case, multiline plain scalar content must be indented more than the indent
            // value.
            ...
            if (key_sep_pos == str_view::npos)
            {
                ...
                const std::size_t block_seq_item_begin_pos =
                    line_indent_part.find_last_of(target_char);
    ...
    /// @note Currently, only %YAML directive is supported. If not, returns invalid or throws an
    /// exception.
        ...
        default: {
            ...
            do
            {
                ...
                default:
                    ...
                    {
                        // See https://yaml.org/spec/1.2.2/#rule-c-named-tag-handle for more
                        // details.
                        emit_error("named handle can contain only numbers(0-9), alphabets(A-Z,a-z) "
                                   "and hyphens(-).");
                    }
                    break;
                }
    ...
    lexical_token_t scan_yaml_version_directive()
    {
        ...
        if FK_YAML_UNLIKELY (m_yaml_version.compare("1.1") != 0 &&
                             m_yaml_version.compare("1.2") != 0)
        {
    ...
    /// @brief Determines the range of single quoted scalar by scanning remaining input buffer
    /// contents.
    ...
    str_view determine_single_quoted_scalar_range()
    {
        ...
        while (pos != str_view::npos)
        {
            ...
            // If single quotation marks are repeated twice in a single quoted scalar, they are
            // considered as an escaped single quotation mark. Skip the second one which would
            // otherwise be detected as a closing single quotation mark in the next loop.
            pos = sv.find('\'', pos + 2);
        }
    ...
    /// @brief Determines the range of double quoted scalar by scanning remaining input buffer
    /// contents.
    ...
    str_view determine_double_quoted_scalar_range()
    {
        ...
        while (pos != str_view::npos)
        {
            ...
            {
                // Double quotation marks can be escaped by a preceding backslash and the number of
                // backslashes matters to determine if the found double quotation mark is escaped
                // since the backslash itself can also be escaped:
        ...
        do
        {
            ...
            case '\n': {
                ...
                if (non_space_pos == str_view::npos ||
                    non_space_pos - last_newline_pos - 1 <= indent)
                {
                    ends_loop = true;
                    break;
                }
            ...
            case ' ':
                ...
                // Allow a space in a plain scalar only if the space is surrounded by non-space
                // characters, but not followed by the comment prefix " #". Also, flow indicators
                // are not allowed to be followed after a space in a flow context. See
                // https://yaml.org/spec/1.2.2/#733-plain-style for more details.
                switch (sv[pos + 1])
                {
                case ' ':
                case '\t':
                ...
                case '#':
                    ...
                    break;
                case ':':
                    // " :" is permitted in a plain style string token, but not when followed by a
                    // space.
    ...
    /// @param indicated_indent The indicated indent level in the block scalar header. 0 means it's
    /// not indicated.
    /// @param token Storage for the scanned block scalar range.
    /// @return The content indentation level of the block scalar.
    str_view determine_block_scalar_content_range(uint32_t base_indent, uint32_t indicated_indent,
                                                  uint32_t &content_indent)
    {
        ...
        while (cur_itr != m_end_itr)
        {
            ...
            case '\t':
                ...
                // See https://yaml.org/spec/1.2.2/#rule-s-indent and
                // https://yaml.org/spec/1.2.2/#64-empty-lines.
                stop_increment = true;
                continue;
            case '\n':
        ...
        {
            // Without the following iterator update, lexer cannot reach the end of input buffer and
            // causes infinite loops from the next loop. (https://github.com/fktn-k/fkYAML/pull/410)
            m_cur_itr = m_end_itr;

            // If there's no non-empty line, the content indentation level is equal to the number of
            // spaces on the longest line.
            // https://yaml.org/spec/1.2.2/#8111-block-indentation-indicator
            content_indent = indicated_indent == 0 ? std::max(cur_indent, max_leading_indent)
                                                   : base_indent + indicated_indent;
            return sv;
        }
        ...
        {
            emit_error(
                "Any leading empty line must not be more indented than the first non-empty line.");
        }
        ...
        while (last_newline_pos < remain_input_len)
        {
            ...
            const std::size_t cur_line_content_begin_pos =
                sv.find_first_not_of(' ', last_newline_pos + 1);
    ...
    block_scalar_header convert_to_block_scalar_header(str_view line)
    {
        ...
        for (const char c : line)
        {
            ...
            case '9':
                ...
                {
                    emit_error("Invalid indentation level for a block scalar. It must be between 1 "
                               "and 9.");
                }
    ...
    {
        m_cur_itr =
            std::find_if_not(m_cur_itr, m_end_itr, [](char c) { return (c == ' ' || c == '\t'); });
    }
    ...
    {
        m_pos_tracker.update_position(m_cur_itr);
        throw fkyaml::parse_error(msg, m_pos_tracker.get_lines_read(),
                                  m_pos_tracker.get_cur_pos_in_line());
    }
...
// Implementations in this header file are intentionally optimized for conversions between YAML
// scalars and native C++ types. So, some implementations don't follow the convensions in the
// standard C++ functions. For example, octals must begin with "0o" (not "0"), which is specified in
// the YAML spec 1.2.
...
template <std::size_t NumBytes> struct conv_limits_base
{
};

/// @brief The specialization of conv_limits_base for 1 byte integers, e.g., int8_t, uint8_t.
template <> struct conv_limits_base<1u>
{
...
template <> struct conv_limits_base<2u>
{
...
template <> struct conv_limits_base<4u>
{
...
template <> struct conv_limits_base<8u>
{
...
template <std::size_t NumBytes, bool IsSigned> struct conv_limits
{
};

/// @brief The specialization of conv_limits for 1 byte signed integers, e.g., int8_t.
template <> struct conv_limits<1u, true> : conv_limits_base<1u>
{
    ...
    static const char *max_value_chars_dec() noexcept
    {
        ...
        // Although the issue has been fixed since C++17, this workaround is necessary to let this
        // functionality work with C++11 (the library's default C++ standard version). The same
        // thing is applied to similar functions in the other specializations.
...
template <> struct conv_limits<1u, false> : conv_limits_base<1u>
{
    /// with or without sign.
...
template <> struct conv_limits<2u, true> : conv_limits_base<2u>
{
    /// with or without sign.
    static constexpr bool is_signed = true;
...
template <> struct conv_limits<2u, false> : conv_limits_base<2u>
{
    /// with or without sign.
    static constexpr bool is_signed = false;
...
template <> struct conv_limits<4u, true> : conv_limits_base<4u>
{
    /// with or without sign.
    static constexpr bool is_signed = true;
...
template <> struct conv_limits<4u, false> : conv_limits_base<4u>
{
    /// with or without sign.
    static constexpr bool is_signed = false;
...
template <> struct conv_limits<8u, true> : conv_limits_base<8u>
{
    /// with or without sign.
    static constexpr bool is_signed = true;
...
template <> struct conv_limits<8u, false> : conv_limits_base<8u>
{
    /// with or without sign.
    static constexpr bool is_signed = false;
...
inline bool atob(CharItr begin, CharItr end, BoolType &boolean) noexcept
{
    ...
    if (len == 4)
    {
        const bool is_true = (std::strncmp(p_begin, "true", 4) == 0) ||
    ...
    if (len == 5)
    {
        const bool is_false = (std::strncmp(p_begin, "false", 5) == 0) ||
...
/// @brief Converts a scalar into decimals. This is common implementation for both signed/unsigned
/// integer types.
/// @warning
/// This function does NOT care about overflows if IntType is unsigned. The source string value must
/// be validated beforehand by calling either atoi_dec_pos() or atoi_dec_neg() functions.
...
{
    static_assert(is_non_bool_integral<IntType>::value,
...
/// @brief Converts a scalar into positive decimals. This function executes bounds check to avoid
/// overflow.
...
{
    static_assert(is_non_bool_integral<IntType>::value,
                  "atoi_dec_pos() accepts non-boolean integral types as an output type");
...
/// @brief Converts a scalar into negative decimals. This function executes bounds check to avoid
/// underflow.
/// @warning `p_begin` and `p_end` must not be null. Validate them before calling this function.
...
{
    static_assert(is_non_bool_integral<IntType>::value,
                  "atoi_dec_neg() accepts non-boolean integral types as an output type");
...
{
    static_assert(is_non_bool_integral<IntType>::value,
                  "atoi_oct() accepts non-boolean integral types as an output type");
...
/// @brief Converts a scalar into hexadecimals. This function executes bounds check to avoid
/// overflow.
/// @warning `p_begin` and `p_end` must not be null. Validate them before calling this function.
/// @tparam IntType The output integer type. It can be either signed or unsigned.
/// @param p_begin The pointer to the first element of the scalar.
...
{
    static_assert(is_non_bool_integral<IntType>::value,
                  "atoi_hex() accepts non-boolean integral types as an output type");
...
/// @brief Converts a scalar into integers. This function executes bounds check to avoid
/// overflow/underflow.
/// @tparam CharItr The type of char iterators. Its value type must be char (maybe cv-qualified).
/// @tparam IntType The output integer type. It can be either signed or unsigned.
/// @param begin The iterator to the first element of the scalar.
...
inline bool atoi(CharItr begin, CharItr end, IntType &i) noexcept
{
    ...
    static_assert(is_non_bool_integral<IntType>::value,
                  "atoi() accepts non-boolean integral types as an output type");
...
inline bool atof_impl(const char *p_begin, const char *p_end, FloatType &f) noexcept
{
    static_assert(std::is_floating_point_v<FloatType>,
                  "atof_impl() accepts floating point types as an output type");
...
inline bool atof(CharItr begin, CharItr end,
                 FloatType &f) noexcept(noexcept(atof_impl(&*begin, &*begin, f)))
{
    static_assert(is_iterator_of<CharItr, char>::value, "atof() accepts iterators for char type");
    static_assert(std::is_floating_point<FloatType>::value,
                  "atof() accepts floating point types as an output type");
    ...
    if (*p_begin == '-' || *p_begin == '+')
    {
        if (len == 5)
        {
            ...
            if (is_inf)
            {
                set_infinity(f, *p_begin == '-' ? static_cast<FloatType>(-1.)
    ...
    {
        const bool is_inf = (std::strncmp(p_begin, ".inf", 4) == 0) ||
        ...
        const bool is_nan = (std::strncmp(p_begin, ".nan", 4) == 0) ||
...
class yaml_escaper
{
    ...
    static ::std::string escape(const char *begin, const char *end, bool &is_escaped)
    {
        ...
        for (; begin != end; ++begin)
        {
            ...
            default:
                ...
                if (diff > 1)
                {
                    if (*begin == static_cast<char>(0xC2u) &&
                        *(begin + 1) == static_cast<char>(0x85u))
                    {
                        escaped += "\\N";
                        std::advance(begin, 1);
                        is_escaped = true;
                    ...
                    if (*begin == static_cast<char>(0xC2u) &&
                        *(begin + 1) == static_cast<char>(0xA0u))
                    {
                        escaped += "\\_";
                        std::advance(begin, 1);
                        is_escaped = true;
                    ...
                    if (diff > 2)
                    {
                        if (*begin == static_cast<char>(0xE2u) &&
                            *(begin + 1) == static_cast<char>(0x80u) &&
  ...
  private:
    ...
    static bool extract_codepoint(const char *&begin, const char *end, int bytes_to_read,
                                  char32_t &codepoint)
    {
        const bool has_enough_room =
            static_cast<int>(std::distance(begin, end)) >= (bytes_to_read - 1);
        if (!has_enough_room)
        {
            return false;
        }
...
class scalar_scanner
{
    ...
    static node_type scan(const char *begin, const char *end) noexcept
    {
        ...
        case 4:
            ...
            case 'n':
                ...
                return (std::strncmp(p_begin + 1, "ull", 3) == 0) ? node_type::NULL_OBJECT
                                                                  : node_type::STRING;
            case 'N':
                // no possible case of begin a number otherwise.
                return ((std::strncmp(p_begin + 1, "ull", 3) == 0) ||
                        (std::strncmp(p_begin + 1, "ULL", 3) == 0))
                           ? node_type::NULL_OBJECT
                           : node_type::STRING;
            case 't':
                // no possible case of being a number otherwise.
                return (std::strncmp(p_begin + 1, "rue", 3) == 0) ? node_type::BOOLEAN
                                                                  : node_type::STRING;
            case 'T':
                // no possible case of being a number otherwise.
                return ((std::strncmp(p_begin + 1, "rue", 3) == 0) ||
                        (std::strncmp(p_begin + 1, "RUE", 3) == 0))
                           ? node_type::BOOLEAN
                           : node_type::STRING;
            case '.': {
                const char *p_from_second = p_begin + 1;
                const bool is_inf_or_nan_scalar = (std::strncmp(p_from_second, "inf", 3) == 0) ||
                                                  (std::strncmp(p_from_second, "Inf", 3) == 0) ||
                                                  (std::strncmp(p_from_second, "INF", 3) == 0) ||
                                                  (std::strncmp(p_from_second, "nan", 3) == 0) ||
                                                  (std::strncmp(p_from_second, "NaN", 3) == 0) ||
                                                  (std::strncmp(p_from_second, "NAN", 3) == 0);
        ...
        case 5:
            ...
            case 'f':
                // no possible case of being a number otherwise.
                return (std::strncmp(p_begin + 1, "alse", 4) == 0) ? node_type::BOOLEAN
                                                                   : node_type::STRING;
            case 'F':
                // no possible case of being a number otherwise.
                return ((std::strncmp(p_begin + 1, "alse", 4) == 0) ||
                        (std::strncmp(p_begin + 1, "ALSE", 4) == 0))
                           ? node_type::BOOLEAN
                           : node_type::STRING;
            case '+':
            case '-':
  ...
  private:
    /// @brief Detects a scalar value type from the contents (possibly an integer or a
    ...
    static node_type scan_after_zero_at_first(const char *itr, uint32_t len) noexcept
    {
        ...
        if (is_digit(*itr))
        {
            // a token consisting of the beginning '0' and some following numbers, e.g., `0123`, is
            // not an integer according to https://yaml.org/spec/1.2.2/#10213-integer.
            return node_type::STRING;
        }
    ...
    /// @brief Detects a scalar value type by scanning the contents right after the exponent prefix
    /// ("e" or "E").
    ...
    /// @note This function is needed to avoid assertion failures in `std::isdigit()` especially
    ...
    /// @note This function is needed to avoid assertion failures in `std::isxdigit()` especially
    /// when compiled with MSVC.
    /// @param c A character to be checked.
...
template <typename BasicNodeType> class scalar_parser
{
    static_assert(is_basic_node<BasicNodeType>::value,
                  "scalar_parser only accepts basic_node<...>");
  ...
  public:
    ...
    scalar_parser(uint32_t line, uint32_t indent) noexcept : m_line(line), m_indent(indent)
    {
    }
    ...
    scalar_parser &operator=(scalar_parser &&) noexcept(
    ...
    {
        FK_YAML_ASSERT(lex_type == lexical_token_t::PLAIN_SCALAR ||
                       lex_type == lexical_token_t::SINGLE_QUOTED_SCALAR ||
    ...
    basic_node_type parse_block(lexical_token_t lex_type, tag_t tag_type, str_view token,
                                const block_scalar_header &header)
    {
        FK_YAML_ASSERT(lex_type == lexical_token_t::BLOCK_LITERAL_SCALAR ||
                       lex_type == lexical_token_t::BLOCK_FOLDED_SCALAR);
        FK_YAML_ASSERT(tag_type != tag_t::SEQUENCE && tag_type != tag_t::MAPPING);
  ...
  private:
        ...
        do
        {
            ...
            if (token[pos] == '\\')
            {
                ...
                if (token[pos + 1] != '\n')
                {
                    ...
                    const bool is_valid_escaping =
                        yaml_escaper::unescape(p_escape_begin, token.end(), m_buffer);
                    if FK_YAML_UNLIKELY (!is_valid_escaping)
                    {
                        throw parse_error(
                            "Unsupported escape sequence is found in a double quoted scalar.",
                            m_line, m_indent);
                    }
        ...
        do
        {
            ...
            else
            {
                ...
                const bool is_more_indented =
                    (non_space_pos != str_view::npos) && (non_space_pos > header.indent);
    ...
    /// @brief Discards final content line break and trailing empty lines depending on the given
    /// chomping type.
    ...
    void process_chomping(chomping_indicator_t chomp)
    {
        ...
        case chomping_indicator_t::STRIP: {
            ...
            {
                // if the scalar has no content line, all lines are considered as trailing empty
                // lines.
                m_buffer.clear();
                break;
            }
    ...
    void process_line_folding(str_view &token, std::size_t newline_pos) noexcept
    {
        ...
        const std::size_t last_non_space_pos =
            token.substr(0, newline_pos + 1).find_last_not_of(" \t");
    ...
    node_type decide_value_type(lexical_token_t lex_type, tag_t tag_type,
                                str_view token) const noexcept
    {
    ...
    basic_node_type create_scalar_node(node_type val_type, tag_t tag_type, str_view token)
    {
        ...
        case node_type::INTEGER: {
            ...
            // conversion error from a scalar which is not tagged with !!int is recovered by
            // treating it as a string scalar. See https://github.com/fktn-k/fkYAML/issues/428.
        ...
        case node_type::FLOAT: {
            ...
            {
                throw parse_error("Failed to convert a scalar to a floating point value", m_line,
                                  m_indent);
            }

            // conversion error from a scalar which is not tagged with !!float is recovered by
            // treating it as a string scalar. See https://github.com/fktn-k/fkYAML/issues/428.
            return basic_node_type(string_type(token.begin(), token.end()));
        }
        case node_type::STRING:
...
template <typename BasicNodeType> class tag_resolver
{
    static_assert(is_basic_node<BasicNodeType>::value,
                  "tag_resolver only accepts basic_node<...>.");
    using doc_metainfo_type = document_metainfo<BasicNodeType>;

  public:
    /// @brief Resolve the input tag name into an expanded tag name prepended with a registered
    /// prefix.
    /// @param tag The input tag name.
    /// @return The type of a node deduced from the given tag name.
    static tag_t resolve_tag(const std::string &tag,
                             const std::shared_ptr<doc_metainfo_type> &directives)
    {
  ...
  private:
    static std::string normalize_tag_name(const std::string &tag,
                                          const std::shared_ptr<doc_metainfo_type> &directives)
    {
        ...
        if (tag.size() == 1)
        {
            ...
            // See the "Non-Specific Tags" section in https://yaml.org/spec/1.2.2/#691-node-tags.
            // The interpretation cannot take place here because the input lacks the corresponding
            // value.
            return tag;
        }
        ...
        case '!': {
            ...
            const bool is_null_or_empty =
                !directives || directives->secondary_handle_prefix.empty();
            if (is_null_or_empty)
            {
                normalized.append(default_secondary_handle_prefix.begin(),
                                  default_secondary_handle_prefix.end());
        ...
        case '<':
            ...
            {
                const bool is_null_or_empty =
                    !directives || directives->primary_handle_prefix.empty();
                if (is_null_or_empty)
                {
                    normalized.append(default_primary_handle_prefix.begin(),
                                      default_primary_handle_prefix.end());
        ...
        default: {
            ...
            if (tag_end_pos != std::string::npos)
            {
                ...
                auto named_handle_itr =
                    directives->named_handle_map.find(tag.substr(0, tag_end_pos + 1));
                ...
                // The YAML spec prohibits expanding the percent-encoded characters (%xx -> a UTF-8
                // byte). So no conversion takes place. See
                // https://yaml.org/spec/1.2.2/#56-miscellaneous-characters for more details.

                normalized += named_handle_itr->second;
                normalized.append(tag.begin() + (static_cast<std::ptrdiff_t>(tag_end_pos) + 1),
                                  tag.end());
                break;
            }
...
/// @tparam T A target type.
template <typename T> using get_buffer_view_fn_t = decltype(std::declval<T>().get_buffer_view());
...
/// @tparam typename N/A
template <typename InputAdapterType, typename = void> struct has_get_buffer_view : std::false_type
{
};

/// @brief A partial specialization of has_get_buffer_view if InputAdapterType has get_buffer_view
/// member function.
/// @tparam InputAdapterType A type of a target input adapter.
template <typename InputAdapterType>
struct has_get_buffer_view<InputAdapterType,
                           enable_if_t<is_detected<get_buffer_view_fn_t, InputAdapterType>::value>>
    : std::true_type
{
};
...
/// @tparam T A target type.
/// @tparam typename N/A
template <typename T, typename = void> struct is_input_adapter : std::false_type
{
};
...
template <typename InputAdapterType>
struct is_input_adapter<InputAdapterType, enable_if_t<has_get_buffer_view<InputAdapterType>::value>>
    : std::true_type
{
};
...
template <typename BasicNodeType> class basic_deserializer
{
    static_assert(is_basic_node<BasicNodeType>::value,
                  "basic_deserializer only accepts basic_node<...>");
    ...
    struct parse_context
    {
        ...
        parse_context(uint32_t line, uint32_t indent, context_state_t state,
                      basic_node_type *p_node) noexcept
            : line(line), indent(indent), state(state), p_node(p_node)
        {
        }
  ...
  public:
    ...
    template <typename InputAdapterType,
              enable_if_t<is_input_adapter<InputAdapterType>::value, int> = 0>
  ...
  private:
    ...
    basic_node_type deserialize_document(lexer_type &lexer, lexical_token_t &last_type)
    {
        ...
        case lexical_token_t::SEQUENCE_BLOCK_PREFIX: {
            ...
            if (found_props)
            {
                // If node properties are found before the block sequence entry prefix, the
                // properties belong to the root sequence node.
                apply_node_properties(root);
            }

            parse_context context(lexer.get_lines_processed(), lexer.get_last_token_begin_pos(),
                                  context_state_t::BLOCK_SEQUENCE, &root);
        ...
        case lexical_token_t::SEQUENCE_FLOW_BEGIN:
            ...
            apply_directive_set(root);
            apply_node_properties(root);
            m_context_stack.emplace_back(lexer.get_lines_processed(),
                                         lexer.get_last_token_begin_pos(),
                                         context_state_t::FLOW_SEQUENCE, &root);
            token = lexer.get_next_token();
            line = lexer.get_lines_processed();
            indent = lexer.get_last_token_begin_pos();
            break;
        case lexical_token_t::MAPPING_FLOW_BEGIN:
            ...
            m_context_stack.emplace_back(lexer.get_lines_processed(),
                                         lexer.get_last_token_begin_pos(),
                                         context_state_t::FLOW_MAPPING, &root);
            token = lexer.get_next_token();
            line = lexer.get_lines_processed();
            indent = lexer.get_last_token_begin_pos();
            break;
        case lexical_token_t::EXPLICIT_KEY_PREFIX: {
            // If the explicit key prefix (? ) is detected here, the root node of current document
            // must be a mapping. Also, tag and anchor if any are associated to the root mapping
            // node. No get_next_token() call here to handle the token event in the
            // deserialize_node() function.
            root = basic_node_type::mapping();
            apply_directive_set(root);
            apply_node_properties(root);
            parse_context context(lexer.get_lines_processed(), lexer.get_last_token_begin_pos(),
                                  context_state_t::BLOCK_MAPPING, &root);
            m_context_stack.emplace_back(std::move(context));
            line = lexer.get_lines_processed();
            indent = lexer.get_last_token_begin_pos();
            break;
        ...
        case lexical_token_t::ALIAS_PREFIX:
            // Defer handling the above token events until the next call on the deserialize_scalar()
            // function since the meaning depends on subsequent events.
            if (found_props && line < lexer.get_lines_processed())
            {
                // If node properties and a followed node are on the different line, the properties
        ...
        FK_YAML_ASSERT(last_type == lexical_token_t::END_OF_BUFFER ||
                       last_type == lexical_token_t::END_OF_DIRECTIVES ||
    ...
    void deserialize_directives(lexer_type &lexer, lexical_token &last_token)
    {
        ...
        for (;;)
        {
            ...
            case lexical_token_t::YAML_VER_DIRECTIVE:
                ...
                {
                    throw parse_error("YAML version cannot be specified more than once.",
            ...
            case lexical_token_t::TAG_DIRECTIVE: {
                ...
                case 1 /* ! */: {
                    ...
                    {
                        throw parse_error("Primary handle cannot be specified more than once.",
                                          lexer.get_lines_processed(),
                                          lexer.get_last_token_begin_pos());
                    }
                ...
                case 2 /* !! */: {
                    ...
                    if FK_YAML_UNLIKELY (is_already_specified)
                    {
                        throw parse_error("Secondary handle cannot be specified more than once.",
                                          lexer.get_lines_processed(),
                                          lexer.get_last_token_begin_pos());
                    }
                    const str_view tag_prefix = lexer.get_tag_prefix();
                ...
                default /* !<handle>! */: {
                    ...
                    const bool is_already_specified =
                        !(mp_meta->named_handle_map
                              .emplace(std::move(tag_handle), std::move(tag_prefix))
                              .second);
                    if FK_YAML_UNLIKELY (is_already_specified)
                    {
                        throw parse_error(
                            "The same named handle cannot be specified more than once.",
                            lexer.get_lines_processed(), lexer.get_last_token_begin_pos());
                    }
                    lacks_end_of_directives_marker = true;
                    break;
                }
    ...
    void deserialize_node(lexer_type &lexer, const lexical_token &first_token, uint32_t first_line,
                          uint32_t first_indent, lexical_token_t &last_type)
    {
        ...
        do
        {
            ...
            {
            case lexical_token_t::EXPLICIT_KEY_PREFIX: {
                const bool needs_to_move_back =
                    indent == 0 || indent < m_context_stack.back().indent;
                ...
                case context_state_t::BLOCK_SEQUENCE_ENTRY:
                    ...
                    *mp_current_node = basic_node_type::mapping();
                    apply_directive_set(*mp_current_node);
                    m_context_stack.emplace_back(line, indent, context_state_t::BLOCK_MAPPING,
                                                 mp_current_node);
                    break;
                default:
                    break;
                ...
                {
                    // heap-allocated node will be freed in handling the corresponding KEY_SEPARATOR
                    // event
                    auto *p_node = new basic_node_type(node_type::SEQUENCE);
                    m_context_stack.emplace_back(
                        line, indent, context_state_t::BLOCK_MAPPING_EXPLICIT_KEY, p_node);

                    apply_directive_set(*p_node);
                    parse_context context(lexer.get_lines_processed(),
                                          lexer.get_last_token_begin_pos(),
                                          context_state_t::BLOCK_SEQUENCE, p_node);
                    m_context_stack.emplace_back(context);

                    p_node->template get_value_ref<sequence_type &>().emplace_back(
                        basic_node_type());
                ...
                // heap-allocated node will be freed in handling the corresponding KEY_SEPARATOR
                // event
                m_context_stack.emplace_back(line, indent,
            ...
            case lexical_token_t::KEY_SEPARATOR: {
                ...
                if FK_YAML_UNLIKELY (m_context_stack.back().state ==
                ...
                const bool is_implicit_same_line =
                    (line == old_line) &&
                    (m_context_stack.empty() || old_indent > m_context_stack.back().indent);
                ...
                if (line > old_line)
                {
                    if (m_needs_tag_impl)
                    {
                        ...
                        {
                            // set YAML node properties here to distinguish them from those for the
                            // first key node as shown in the following snippet:
                            //
                            // ```yaml
                            // foo: !!map
                            //   !!str 123: true
                    ...
                    if (token.type == lexical_token_t::SEQUENCE_BLOCK_PREFIX)
                    {
                        ...
                        *mp_current_node = basic_node_type::sequence({basic_node_type()});
                        ...
                        mp_current_node =
                            &(mp_current_node->template get_value_ref<sequence_type &>().back());
                        apply_directive_set(*mp_current_node);
                        ...
                        if (has_props)
                        {
                            ...
                            {
                                // Skip updating the current indent to avoid stacking a wrong
                                // indentation.
                                //
                                // ```yaml
                                // &foo bar: baz
                            ...
                            // if node properties and the followed node are on different lines
                            // (i.e., the properties are for a container node), the application and
                            // the line advancement must happen here. Otherwise, a false indent
                            // error will be emitted. See
                            // https://github.com/fktn-k/fkYAML/issues/368 for more details.
                            line = line_after_props;
                            indent = lexer.get_last_token_begin_pos();
                            *mp_current_node = basic_node_type::mapping();
                            m_context_stack.emplace_back(line_after_props, indent,
                                                         context_state_t::BLOCK_MAPPING,
                                                         mp_current_node);
                            apply_directive_set(*mp_current_node);
                            apply_node_properties(*mp_current_node);
                        }
                    ...
                    {
                        FK_YAML_ASSERT(m_context_stack.back().state ==
                                       context_state_t::MAPPING_VALUE);
                        ...
                        // ```
                        pop_to_parent_node(line, indent, [indent](const parse_context &c) {
                            return (c.state == context_state_t::BLOCK_MAPPING) &&
                                   (indent == c.indent);
                        });
                    }

                    // defer checking the existence of a key separator after the following scalar
                    // until the next deserialize_scalar() call.
                    continue;
                }
                ...
                m_context_stack.emplace_back(old_line, old_indent,
                                             context_state_t::BLOCK_MAPPING_EXPLICIT_VALUE,
                                             mp_current_node);
                ...
                if (token.type == lexical_token_t::SEQUENCE_BLOCK_PREFIX)
                {
                    *mp_current_node = basic_node_type::sequence({basic_node_type()});
                    apply_directive_set(*mp_current_node);
                    apply_node_properties(*mp_current_node);
                    m_context_stack.emplace_back(line, indent, context_state_t::BLOCK_SEQUENCE,
                                                 mp_current_node);
            ...
            case lexical_token_t::TAG_PREFIX:
                ...
                // Note that node properties for block sequences as a mapping value are processed
                // when a `lexical_token_t::KEY_SEPARATOR` token is processed.
                //
                // ```yaml
                // &foo bar: baz
                // ^
            ...
            case lexical_token_t::SEQUENCE_BLOCK_PREFIX: {
                ...
                {
                    // If the previous block sequence entry is empty, just move to the parent
                    // context.
                    // ```yaml
                    // foo:
                    //   -
                ...
                apply_directive_set(*mp_current_node);
                m_context_stack.emplace_back(line, indent, context_state_t::BLOCK_SEQUENCE_ENTRY,
                                             mp_current_node);
                break;
            }
            case lexical_token_t::SEQUENCE_FLOW_BEGIN:
                ...
                else if FK_YAML_UNLIKELY (m_flow_token_state ==
                                          flow_token_state_t::NEEDS_SEPARATOR_OR_SUFFIX)
                {
                    throw parse_error(
                        "Flow sequence beginning is found without separated with a comma.", line,
                        indent);
                }
                ...
                case context_state_t::FLOW_SEQUENCE:
                    ...
                    mp_current_node =
                        &(mp_current_node->template get_value_ref<sequence_type &>().back());
                    m_context_stack.emplace_back(line, indent, context_state_t::FLOW_SEQUENCE,
                                                 mp_current_node);
                    break;
                case context_state_t::BLOCK_MAPPING:
                case context_state_t::FLOW_MAPPING:
                    // heap-allocated node will be freed in handling the corresponding
                    // SEQUENCE_FLOW_END event.
                    m_context_stack.emplace_back(line, indent, context_state_t::FLOW_SEQUENCE_KEY,
            ...
            case lexical_token_t::SEQUENCE_FLOW_END: {
                ...
                {
                    throw parse_error("Flow sequence ending is found outside the flow context.",
                                      line, indent);
                }
                ...
                auto itr = std::find_if( // LCOV_EXCL_LINE
                    m_context_stack.rbegin(), m_context_stack.rend(), [](const parse_context &c) {
                        switch (c.state)
                        {
                        case context_state_t::FLOW_SEQUENCE_KEY:
                        case context_state_t::FLOW_SEQUENCE:
                ...
                if FK_YAML_UNLIKELY (!is_valid)
                {
                    throw parse_error("No corresponding flow sequence beginning is found.", line,
                                      indent);
                }
            ...
            case lexical_token_t::MAPPING_FLOW_BEGIN:
                ...
                {
                    throw parse_error(
                        "Flow mapping beginning is found without separated with a comma.", line,
                        indent);
                }
                ...
                case context_state_t::FLOW_SEQUENCE:
                    ...
                    mp_current_node =
                        &(mp_current_node->template get_value_ref<sequence_type &>().back());
                    m_context_stack.emplace_back(line, indent, context_state_t::FLOW_MAPPING,
                                                 mp_current_node);
                    break;
                case context_state_t::BLOCK_MAPPING:
                case context_state_t::FLOW_MAPPING:
                    // heap-allocated node will be freed in handling the corresponding
                    // MAPPING_FLOW_END event.
                    m_context_stack.emplace_back(line, indent, context_state_t::FLOW_MAPPING_KEY,
            ...
            case lexical_token_t::MAPPING_FLOW_END: {
                if FK_YAML_UNLIKELY (m_flow_context_depth == 0)
                {
                    throw parse_error("Flow mapping ending is found outside the flow context.",
                                      line, indent);
                }
                ...
                if FK_YAML_UNLIKELY (!is_valid)
                {
                    throw parse_error("No corresponding flow mapping beginning is found.", line,
                                      indent);
                }
            ...
            case lexical_token_t::VALUE_SEPARATOR:
                ...
                if FK_YAML_UNLIKELY (m_flow_token_state !=
                                     flow_token_state_t::NEEDS_SEPARATOR_OR_SUFFIX)
                {
            ...
            case lexical_token_t::ALIAS_PREFIX: {
                ...
                const auto anchor_counts =
                    static_cast<uint32_t>(mp_meta->anchor_table.count(token_str));
                if FK_YAML_UNLIKELY (anchor_counts == 0)
                {
                    throw parse_error("The given anchor name must appear prior to the alias node.",
                                      line, indent);
                }
            ...
            case lexical_token_t::DOUBLE_QUOTED_SCALAR: {
                ...
                basic_node_type node =
                    scalar_parser_type(line, indent).parse_flow(token.type, tag_type, token.str);
                apply_directive_set(node);
                apply_node_properties(node);
            ...
            case lexical_token_t::BLOCK_FOLDED_SCALAR: {
                ...
                basic_node_type node = scalar_parser_type(line, indent)
                                           .parse_block(token.type, tag_type, token.str,
                                                        lexer.get_block_scalar_header());
                apply_directive_set(node);
                apply_node_properties(node);
    ...
    /// @param lexer The lexical analyzer to be used.
    /// @param last_type The variable to store the last lexical token type.
    /// @param line The variable to store the line of either the first property or the last
    /// non-property token.
    /// @param indent The variable to store the indent of either the first property or the last
    /// non-property token.
    /// @return true if any property is found, false otherwise.
    bool deserialize_node_properties(lexer_type &lexer, lexical_token &last_token, uint32_t &line,
                                     uint32_t &indent)
    {
    ...
    void add_new_key(basic_node_type &&key, const uint32_t line, const uint32_t indent)
    {
        ...
        else
        {
            ...
            {
                throw parse_error("Flow mapping entry is found without separated with a comma.",
                                  line, indent);
            }
        ...
        auto itr = mp_current_node->template get_value_ref<mapping_type &>().emplace(
            std::move(key), basic_node_type());
        ...
        m_context_stack.emplace_back(key_context.line, key_context.indent,
                                     context_state_t::MAPPING_VALUE, mp_current_node);
    }
    ...
    void assign_node_value(basic_node_type &&node_value, const uint32_t line, const uint32_t indent)
    {
        if (mp_current_node->is_sequence())
        {
            ...
            if FK_YAML_UNLIKELY (m_flow_token_state != flow_token_state_t::NEEDS_VALUE_OR_SUFFIX)
            {
                ...
                // [foo,,bar]
                // ```
                throw parse_error("flow sequence entry is found without separated with a comma.",
                                  line, indent);
            }
        ...
        *mp_current_node = std::move(node_value);
        ...
        if FK_YAML_LIKELY (m_context_stack.back().state !=
                           context_state_t::BLOCK_MAPPING_EXPLICIT_KEY)
        {
            m_context_stack.pop_back();
            mp_current_node = m_context_stack.back().p_node;
    ...
    void deserialize_scalar(lexer_type &lexer, basic_node_type &&node, uint32_t &indent,
                            uint32_t &line, lexical_token &token)
    {
        token = lexer.get_next_token();
            ...
            if (mp_current_node->is_scalar())
            {
                if FK_YAML_LIKELY (!m_context_stack.empty())
                {
                    ...
                    default:
                        ...
                        {
                            throw parse_error(
                                "Multiple mapping keys are specified on the same line.", line,
                                indent);
                        }
                        cur_context.line = line;
                        cur_context.indent = indent;
    ...
    template <typename Pred> void pop_to_parent_node(uint32_t line, uint32_t indent, Pred &&pred)
    {
        FK_YAML_ASSERT(!m_context_stack.empty());

        // LCOV_EXCL_START
        auto itr = std::find_if(m_context_stack.rbegin(), m_context_stack.rend(),
    ...
    {
        return (version_str.compare("1.1") == 0) ? yaml_version_type::VERSION_1_1
                                                 : yaml_version_type::VERSION_1_2;
    }
...
/// @note Since fkYAML doesn't treat UTF-16/UTF-32 encoded characters per byte, endians do not
/// matter.
...
inline utf_encode_t detect_encoding_type(const std::array<uint8_t, 4> &bytes,
                                         bool &has_bom) noexcept
{
    has_bom = false;
    ...
    if (byte0 == 0 && byte1 == 0 && byte2 == static_cast<uint8_t>(0xFEu) &&
        byte3 == static_cast<uint8_t>(0xFFu))
    {
        has_bom = true;
...
template <typename ItrType, typename = void> struct utf_encode_detector
{
};
    ...
    static utf_encode_t detect(ItrType &begin, const ItrType &end) noexcept
    {
        ...
        for (int i = 0; i < 4 && begin + i != end; i++)
        {
            bytes[i] = static_cast<uint8_t>(
                begin[i]); // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
        }
...
struct utf_encode_detector<ItrType, enable_if_t<is_iterator_of<ItrType, char8_t>::value>>
{
    ...
    static utf_encode_t detect(ItrType &begin, const ItrType &end)
    {
        ...
        {
            bytes[i] =
                uint8_t(begin[i]); // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
        }
...
struct utf_encode_detector<ItrType, enable_if_t<is_iterator_of<ItrType, char16_t>::value>>
{
    ...
    static utf_encode_t detect(ItrType &begin, const ItrType &end)
    {
        ...
        if FK_YAML_UNLIKELY (encode_type != utf_encode_t::UTF_16BE &&
                             encode_type != utf_encode_t::UTF_16LE)
        {
...
struct utf_encode_detector<ItrType, enable_if_t<is_iterator_of<ItrType, char32_t>::value>>
{
    ...
    static utf_encode_t detect(ItrType &begin, const ItrType &end)
    {
        ...
        if FK_YAML_UNLIKELY (encode_type != utf_encode_t::UTF_32BE &&
                             encode_type != utf_encode_t::UTF_32LE)
        {
...
struct file_utf_encode_detector
{
    ...
    static utf_encode_t detect(std::FILE *p_file) noexcept
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            bytes[i] = static_cast<uint8_t>(
                byte & 0xFF); // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
        }
...
struct stream_utf_encode_detector
{
    ...
    static utf_encode_t detect(std::istream &is) noexcept
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            bytes[i] = static_cast<uint8_t>(
                ch & 0xFF); // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
        }
...
template <typename IterType, typename = void> class iterator_input_adapter;
...
class iterator_input_adapter<IterType, enable_if_t<is_iterator_of<IterType, char>::value>>
{
  public:
    ...
    iterator_input_adapter(IterType begin, IterType end, utf_encode_t encode_type,
                           bool is_contiguous) noexcept
        : m_begin(begin), m_end(end), m_encode_type(encode_type), m_is_contiguous(is_contiguous)
    {
    }
  ...
  private:
    ...
    {
        FK_YAML_ASSERT(m_encode_type == utf_encode_t::UTF_16BE ||
        ...
        while (current != m_end || encoded_buf_size != 0)
        {
            ...
            {
                auto utf16 =
                    static_cast<char16_t>(static_cast<uint8_t>(*current++) << shift_bits[0]);
    ...
    {
        FK_YAML_ASSERT(m_encode_type == utf_encode_t::UTF_32BE ||
    ...
    str_view get_buffer_view()
    {
        ...
        while (current != m_end || encoded_buf_size != 0)
        {
            while (current != m_end && encoded_buf_size < 2)
            {
                ...
                utf16 = static_cast<char16_t>(((utf16 & 0x00FFu) << shift_bits) |
                                              ((utf16 & 0xFF00u) >> shift_bits));
...
class file_input_adapter
{
  public:
    ...
    explicit file_input_adapter(std::FILE *file, utf_encode_t encode_type) noexcept
        : m_file(file), m_encode_type(encode_type)
    {
    }
  ...
  private:
    ...
    str_view get_buffer_view_utf16()
    {
        ...
        while (std::feof(m_file) == 0)
        {
            ...
            {
                const auto utf16 =
                    static_cast<char16_t>((static_cast<uint8_t>(chars[0]) << shift_bits[0]) |
    ...
    str_view get_buffer_view_utf32()
    {
        ...
        while (std::feof(m_file) == 0)
        {
            ...
            const auto utf32 =
                static_cast<char32_t>((static_cast<uint8_t>(chars[0]) << shift_bits[0]) |
                                      (static_cast<uint8_t>(chars[1]) << shift_bits[1]) |
                                      (static_cast<uint8_t>(chars[2]) << shift_bits[2]) |
                                      (static_cast<uint8_t>(chars[3]) << shift_bits[3]));
...
class stream_input_adapter
{
  public:
    ...
    explicit stream_input_adapter(std::istream &is, utf_encode_t encode_type) noexcept
        : m_istream(&is), m_encode_type(encode_type)
    {
    }
...
template <typename ItrType>
inline iterator_input_adapter<ItrType> create_iterator_input_adapter(ItrType begin, ItrType end,
                                                                     bool is_contiguous)
{
...
/// @param begin The beginning of iterators.
/// @param end The end of iterators.
/// @return iterator_input_adapter<ItrType> An iterator_input_adapter object for the target iterator
/// type.
template <typename ItrType>
inline iterator_input_adapter<ItrType> input_adapter(ItrType begin, ItrType end)
{
    constexpr bool is_random_access_itr =
        std::is_same<typename std::iterator_traits<ItrType>::iterator_category,
...
/// @return decltype(input_adapter(array, array + N)) An iterator_input_adapter object for the
/// target array.
template <typename T, std::size_t N>
inline auto input_adapter(T (&array)[N])
    -> decltype(create_iterator_input_adapter(array, array + (N - 1), true))
{
...
/// @tparam typename N/A
template <typename ContainerType, typename = void> struct container_input_adapter_factory
{
};

/// @brief A partial specialization of container_input_adapter_factory if begin()/end() are
/// available for ContainerType.
/// @tparam ContainerType A container type.
...
template <typename ValueType> struct iterator_traits
{
...
/// @tparam ValueType The type of iterated elements.
template <typename ValueType> struct iterator_traits<const ValueType>
{
    /// A type of iterated elements.
    using value_type = typename ValueType::value_type;
    /// A type to represent difference between iterators.
...
template <typename BasicNodeType> struct iterator_holder
{
    static_assert(is_basic_node<BasicNodeType>::value,
...
template <typename ValueType> class iterator
{
  ...
  public:
    ...
    static_assert(is_basic_node<value_type>::value,
                  "iterator class only accepts a basic_node as its value type.");
    ...
    /// @brief Copy constructs an iterator from another iterator with different const-ness in
    /// ValueType.
    /// @note This copy constructor is not defined if ValueType is not const to avoid const removal
    /// from ValueType.
    ...
    template <typename OtherIterator,
              enable_if_t<conjunction<std::is_same<OtherIterator, other_iterator_type>,
    ...
    /// @return pointer A pointer to the BasicNodeType object internally referenced by the actual
    /// iterator object.
    ...
    /// @return reference Reference to the Node object internally referenced by the actual iterator
    /// object.
    ...
    template <typename Iterator,
              enable_if_t<disjunction<std::is_same<Iterator, iterator>,
                                      std::is_same<Iterator, other_iterator_type>>::value,
                          int> = 0>
    ...
    bool operator<(const Iterator &rhs) const
    {
        ...
        if FK_YAML_UNLIKELY (m_inner_iterator_type == iterator_t::MAPPING)
        {
            throw fkyaml::exception(
                "Cannot compare order of iterators of the mapping container type");
        }
...
/// @note This class makes it easier to handle lvalue basic_node objects in basic_node ctor with
/// std::initializer_list.
/// @tparam BasicNodeType A basic_node template instance type.
template <typename BasicNodeType> class node_ref_storage
{
    static_assert(is_basic_node<BasicNodeType>::value,
                  "node_ref_storage only accepts basic_node<...>");
  ...
  public:
    ...
    explicit node_ref_storage(node_type &&n) noexcept(
        std::is_nothrow_move_constructible<node_type>::value)
    ...
    explicit node_ref_storage(const node_type &n) noexcept : m_value_ref(&n)
    {
    }

    /// @brief Construct a new node ref storage object with a std::initializer_list object.
    /// @param init A std::initializer_list object.
    node_ref_storage(std::initializer_list<node_ref_storage> init) : m_owned_value(init)
    {
    }
    ...
    template <typename... Args,
              enable_if_t<std::is_constructible<node_type, Args...>::value, int> = 0>
    node_ref_storage(Args &&...args) : m_owned_value(std::forward<Args>(args)...)
    {
    }
...
template <> inline void to_string(std::nullptr_t /*unused*/, std::string &s) noexcept
{
    s = "null";
}
...
template <> inline void to_string(bool v, std::string &s) noexcept
{
    s = v ? "true" : "false";
}
...
inline enable_if_t<is_non_bool_integral<IntegerType>::value> to_string(IntegerType v,
...
template <typename FloatType>
inline enable_if_t<std::is_floating_point<FloatType>::value> to_string(FloatType v,
                                                                       std::string &s) noexcept
{
    if (std::isnan(v))
    {
    ...
    // If `v` is actually an integer and no scientific notation is used for serialization, ".0" must
    // be appended. The result would cause a roundtrip issue otherwise.
...
template <typename BasicNodeType> class basic_serializer
{
    static_assert(detail::is_basic_node<BasicNodeType>::value,
                  "basic_serializer only accepts basic_node<...>");
  ...
  private:
    ...
    void serialize_node(const BasicNodeType &node, const uint32_t cur_indent, std::string &str)
    {
        ...
        case node_type::MAPPING:
            for (auto itr = node.begin(); itr != node.end(); ++itr)
            {
                ...
                if (is_appended)
                {
                    // The trailing white space is necessary since anchor names can contain a colon
                    // (:) at its end.
                    str += " ";
                }
                else
        ...
        case node_type::NULL_OBJECT:
            ...
            break;
        case node_type::BOOLEAN:
            to_string(node.template get_value<typename BasicNodeType::boolean_type>(),
                      m_tmp_str_buff);
            str += m_tmp_str_buff;
            break;
        case node_type::INTEGER:
            to_string(node.template get_value<typename BasicNodeType::integer_type>(),
                      m_tmp_str_buff);
            str += m_tmp_str_buff;
            break;
        case node_type::FLOAT:
            to_string(node.template get_value<typename BasicNodeType::float_number_type>(),
                      m_tmp_str_buff);
            str += m_tmp_str_buff;
            break;
        case node_type::STRING: {
            ...
            // The next line is intentionally excluded from the LCOV coverage target since the next
            // line is somehow misrecognized as it has a binary branch. Possibly begin() or end()
            // has some conditional branch(es) internally. Confirmed with LCOV 1.14 on Ubuntu22.04.
            const node_type type_if_plain = scalar_scanner::scan(
                str_val.c_str(), str_val.c_str() + str_val.size()); // LCOV_EXCL_LINE
    ...
    typename BasicNodeType::string_type get_string_node_value(const BasicNodeType &node,
                                                              bool &is_escaped)
    {
...
template <typename Iterator> class reverse_iterator
{
  ...
  public:
    ...
    /// @brief The type to represent differences between the pointed-to elements by the base
    /// iterators.
    ...
    reverse_iterator(const Iterator &i) noexcept : m_current(i)
    {
    }

    template <typename U, enable_if_t<negation<std::is_same<U, Iterator>>::value, int> = 0>
    reverse_iterator(const reverse_iterator<U> &other) noexcept : m_current(other.base())
    {
    }
    ...
    /// @brief Advances the underlying iterator object by `n`.
    /// @param n The distance by which the underlying iterator is advanced.
    /// @return Reference to this reverse_iterator object with the underlying iterator advanced by
    /// `n`.
    ...
    /// @brief Decrements the underlying iterator object by `n`.
    /// @param n The distance by which the underlying iterator is decremented.
    /// @return Reference to this reverse_iterator object with the underlying iterator decremented
    /// by `n`.
    reverse_iterator &operator-=(difference_type n) noexcept
    {
        m_current += n;
...
/// @brief Utility traits type alias to detect constructible associative container types from a
/// mapping node, e.g., std::map or std::unordered_map.
...
/// @brief Utility traits type alias to detect constructible container types from a sequence node,
/// e.g., std::vector or std::list.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam T A target type for detection.
template <typename BasicNodeType, typename T>
using is_constructible_sequence_type =
    conjunction<negation<is_basic_node<T>>, detect::has_iterator<T>,
                detect::is_iterator_traits<typename T::iterator>, detect::has_begin_end<T>,
                negation<std::is_same<T, typename BasicNodeType::mapping_type>>,
                negation<is_constructible_mapping_type<T>>>;

/// @brief Utility traits type alias to detect a sequence container adapter type, e.g., std::stack
/// or std::queue.
/// @tparam T A target type for detection.
template <typename T>
using is_sequence_container_adapter =
    conjunction<negation<is_basic_node<T>>, detect::has_container_type<T>,
                detect::has_value_type<T>, negation<detect::has_key_type<T>>>;

/// @brief Helper struct for reserve() member function call switch for types which do not have
/// reserve function.
/// @tparam ContainerType A container type.
template <typename ContainerType, typename = void> struct call_reserve_if_available
{
...
/// @brief Helper struct for reserve() member function call switch for types which have reserve
/// function.
/// @tparam ContainerType A container type.
template <typename ContainerType>
struct call_reserve_if_available<ContainerType,
                                 enable_if_t<detect::has_reserve<ContainerType>::value>>
{
...
/// @brief from_node function for C-style 1D arrays whose element type must be a basic_node template
/// instance type or a compatible type.
/// @tparam BasicNodeType A basic_node template instance type.
    ...
    -> decltype(n.get_value_inplace(std::declval<T &>()), void())
{
    ...
    // call get_value_inplace(), not get_value(), since the storage to fill the result into is
    // already created.
...
/// @brief from_node function for C-style 2D arrays whose element type must be a basic_node template
/// instance type or a compatible type.
/// @tparam BasicNodeType A basic_node template instance type.
...
/// @brief from_node function for std::array objects whose element type must be a basic_node
/// template instance type or a compatible type. This function is necessary since insert function is
/// not implemented for std::array.
/// @tparam BasicNodeType A basic_node template instance type.
...
/// @brief from_node function for std::valarray objects whose element type must be a basic_node
/// template instance type or a compatible type. This function is necessary since insert function is
/// not implemented for std::valarray.
/// @tparam BasicNodeType A basic_node template instance type.
...
/// @brief from_node function for std::forward_list objects whose element type must be a basic_node
/// template instance type or a compatible type. This function is necessary since insert function is
/// not implemented for std::forward_list.
/// @tparam BasicNodeType A basic_node template instance type.
...
/// @brief from_node function for container objects of only keys or values, e.g., std::vector or
/// std::set, whose element type must be a basic_node template instance type or a compatible type.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam CompatSeqType A container type.
/// @param n A basic_node object.
/// @param s A container object.
template <typename BasicNodeType, typename CompatSeqType,
          enable_if_t<conjunction<is_basic_node<BasicNodeType>,
                                  is_constructible_sequence_type<BasicNodeType, CompatSeqType>,
                                  negation<std::is_constructible<
                                      typename BasicNodeType::string_type, CompatSeqType>>>::value,
                      int> = 0>
...
/// @brief from_node function for sequence container adapter objects, e.g., std::stack or
/// std::queue, whose element type must be either a basic_node template instance type or a
/// compatible type.
/// @tparam BasicNodeType A basic_node template instance type.
...
template <typename BasicNodeType, typename SeqContainerAdapter,
          enable_if_t<conjunction<is_basic_node<BasicNodeType>,
                                  is_sequence_container_adapter<SeqContainerAdapter>>::value,
                      int> = 0>
inline auto from_node(const BasicNodeType &n, SeqContainerAdapter &ca)
    -> decltype(n.template get_value<typename SeqContainerAdapter::value_type>(),
                ca.push(std::declval<typename SeqContainerAdapter::value_type>()), void())
{
    ...
    // clear existing elements manually since clear function is not implemeneted for container
    // adapter classes.
    while (!ca.empty())
    {
        ca.pop();
    ...
    for (const auto &elem : n)
    {
        ...
        // emplace function cannot be used in case SeqContainerAdapter::container_type is
        // std::vector<bool> in C++11.
...
template <typename BasicNodeType, typename CompatMapType,
          enable_if_t<is_constructible_mapping_type<CompatMapType>::value, int> = 0>
inline auto from_node(const BasicNodeType &n, CompatMapType &m)
    -> decltype(std::declval<const BasicNodeType &>()
                ...
                void())
{
    ...
    for (const auto &pair :
         n.template get_value_ref<const typename BasicNodeType::mapping_type &>())
    {
        m.emplace(pair.first.template get_value<typename CompatMapType::key_type>(),
...
inline void from_node(const BasicNodeType &n, bool &b)
{
    ...
    case node_type::BOOLEAN:
        b = static_cast<bool>(
            n.template get_value_ref<const typename BasicNodeType::boolean_type &>());
        break;
    case node_type::INTEGER:
    ...
    case node_type::STRING:
    default:
        throw type_error("The target node value type is not compatible with boolean type.",
                         n.get_type());
    }
}
...
template <typename BasicNodeType, typename IntType,
          bool = std::is_same<typename BasicNodeType::integer_type, IntType>::value>
...
struct from_node_int_helper<BasicNodeType, IntType, false>
{
    ...
    static IntType convert(const BasicNodeType &n)
    {
        ...
        {
            if FK_YAML_UNLIKELY (tmp_int <
                                 static_cast<node_int_type>(std::numeric_limits<IntType>::min()))
            {
                throw exception("Integer value underflow detected.");
            }
            if FK_YAML_UNLIKELY (static_cast<node_int_type>(std::numeric_limits<IntType>::max()) <
                                 tmp_int)
            {
...
/// @note If node's value is null, boolean, or float, such a value is converted into an integer
/// internally.
/// @tparam BasicNodeType A basic_node template instance type.
...
template <
    ...
    enable_if_t<conjunction<is_basic_node<BasicNodeType>, is_non_bool_integral<IntegerType>>::value,
                int> = 0>
inline void from_node(const BasicNodeType &n, IntegerType &i)
{
    ...
    case node_type::FLOAT: {
        ...
        const auto tmp_int = static_cast<int64_t>(
            n.template get_value_ref<const typename BasicNodeType::float_number_type &>());
        ...
        {
            if FK_YAML_UNLIKELY (tmp_int <
                                 static_cast<int64_t>(std::numeric_limits<IntegerType>::min()))
            {
                throw exception("Integer value underflow detected.");
            }
            if FK_YAML_UNLIKELY (static_cast<int64_t>(std::numeric_limits<IntegerType>::max()) <
                                 tmp_int)
            {
                throw exception("Integer value overflow detected.");
            }
    ...
    case node_type::STRING:
    default:
        throw type_error("The target node value type is not compatible with integer type.",
                         n.get_type());
    }
}

/// @brief Helper struct for node-to-float conversion if FloatType is the node's floating point
/// value type.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam FloatType Target floating point value type (same as the
/// BasicNodeType::float_number_type)
...
/// @brief Helper struct for node-to-float conversion if IntType is not the node's floating point
/// value type.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam FloatType Target floating point value type (different from
/// BasicNodeType::float_number_type)
template <typename BasicNodeType, typename FloatType>
...
/// @note If node's value is null, boolean, or integer, such a value is converted into a floating
/// point internally.
/// @tparam BasicNodeType A basic_node template instance type.
...
template <
    ...
    enable_if_t<conjunction<is_basic_node<BasicNodeType>, std::is_floating_point<FloatType>>::value,
                int> = 0>
inline void from_node(const BasicNodeType &n, FloatType &f)
{
    ...
    case node_type::BOOLEAN:
        ...
        break;
    case node_type::INTEGER:
        f = static_cast<FloatType>(
            n.template get_value_ref<const typename BasicNodeType::integer_type &>());
        break;
    case node_type::FLOAT:
    ...
    case node_type::STRING:
    default:
        throw type_error("The target node value type is not compatible with float number type.",
                         n.get_type());
    }
}
...
template <
    ...
    enable_if_t<
        conjunction<
            ...
            disjunction<std::is_constructible<CompatibleStringType,
                                              const typename BasicNodeType::string_type &>,
                        std::is_assignable<CompatibleStringType,
                                           const typename BasicNodeType::string_type &>>>::value,
        int> = 0>
...
/// template instance type or a compatible type.
/// @tparam BasicNodeType A basic_node template instance type.
...
template <typename BasicNodeType, typename T, typename U,
          enable_if_t<is_basic_node<BasicNodeType>::value, int> = 0>
inline auto from_node(const BasicNodeType &n, std::pair<T, U> &p)
    -> decltype(std::declval<const BasicNodeType &>().template get_value<T>(),
...
inline std::tuple<Types...> from_node_tuple_impl(const BasicNodeType &n,
                                                 index_sequence<Idx...> /*unused*/)
{
    return std::make_tuple(n.at(Idx).template get_value<Types>()...);
}

/// @brief from_node function for std::tuple objects whose value types must all be either a
/// basic_node template instance type or a compatible type
/// @tparam BasicNodeType A basic_node template instance type.
...
inline void from_node(const BasicNodeType &n, std::tuple<Types...> &t)
{
    ...
    // Types... must be explicitly specified; the retun type would otherwise be std::tuple with no
    // value types.
...
/// @brief from_node function for std::optional objects whose value type must be either a basic_node
/// template instance type or a compatible type.
/// @tparam BasicNodeType A basic_node template instance type.
...
template <typename BasicNodeType, typename T,
          enable_if_t<is_basic_node<BasicNodeType>::value, int> = 0>
inline auto from_node(const BasicNodeType &n, std::optional<T> &o)
    -> decltype(n.template get_value<T>(), void())
{
    try
    {
    ...
    {
        // Any exception derived from std::exception is interpreted as a conversion failure in some
        // way since user-defined from_node function may throw a different object from a
        // fkyaml::type_error. and std::exception is usually the base class of user-defined
        // exception types.
        o = std::nullopt;
    }
}
...
/// @note User-defined specialization is available by providing implementation **OUTSIDE** fkyaml
/// namespace.
struct from_node_fn
{
    ...
    /// @return decltype(from_node(n, std::forward<T>(val))) void by default. User can set it to
    /// some other type.
    template <typename BasicNodeType, typename T>
    auto operator()(const BasicNodeType &n, T &&val) const
        noexcept(noexcept(from_node(n, std::forward<T>(val))))
...
FK_YAML_INLINE_VAR constexpr const auto &from_node =
    detail::static_const<detail::from_node_fn>::value;
...
/// @note All the non-specialized instantiations results in compilation error since such
/// instantiations are not supported.
/// @warning All the specialization must call n.m_node_value.destroy() first in the construct
/// function to avoid memory leak.
/// @tparam node_type The resulting YAML node value type.
template <node_type> struct external_node_constructor;

/// @brief The specialization of external_node_constructor for sequence nodes.
template <> struct external_node_constructor<node_type::SEQUENCE>
{
    ...
    static void construct(BasicNodeType &n, const typename BasicNodeType::sequence_type &s) noexcept
    {
        ...
        n.m_node_value.p_sequence =
            BasicNodeType::template create_object<typename BasicNodeType::sequence_type>(s);
    }
    ...
    static void construct(BasicNodeType &n, typename BasicNodeType::sequence_type &&s) noexcept
    {
        ...
        n.m_attrs = detail::node_attr_bits::seq_bit;
        n.m_node_value.p_sequence =
            BasicNodeType::template create_object<typename BasicNodeType::sequence_type>(
                std::move(s));
    }
};

/// @brief The specialization of external_node_constructor for mapping nodes.
template <> struct external_node_constructor<node_type::MAPPING>
{
    ...
    static void construct(BasicNodeType &n, const typename BasicNodeType::mapping_type &m) noexcept
    {
        ...
        n.m_node_value.p_mapping =
            BasicNodeType::template create_object<typename BasicNodeType::mapping_type>(m);
    }
    ...
    static void construct(BasicNodeType &n, typename BasicNodeType::mapping_type &&m) noexcept
    {
        ...
        n.m_attrs = detail::node_attr_bits::map_bit;
        n.m_node_value.p_mapping =
            BasicNodeType::template create_object<typename BasicNodeType::mapping_type>(
                std::move(m));
    }
};

/// @brief The specialization of external_node_constructor for null nodes.
template <> struct external_node_constructor<node_type::NULL_OBJECT>
{
...
template <> struct external_node_constructor<node_type::BOOLEAN>
{
...
template <> struct external_node_constructor<node_type::INTEGER>
{
...
template <> struct external_node_constructor<node_type::FLOAT>
{
...
template <> struct external_node_constructor<node_type::STRING>
{
    ...
    static void construct(BasicNodeType &n, const typename BasicNodeType::string_type &s) noexcept
    {
        ...
        n.m_node_value.p_string =
            BasicNodeType::template create_object<typename BasicNodeType::string_type>(s);
    }
    ...
    static void construct(BasicNodeType &n, typename BasicNodeType::string_type &&s) noexcept
    {
        ...
        n.m_attrs = detail::node_attr_bits::string_bit;
        n.m_node_value.p_string =
            BasicNodeType::template create_object<typename BasicNodeType::string_type>(
                std::move(s));
    }
    ...
    template <typename BasicNodeType, typename CompatibleStringType,
              enable_if_t<conjunction<is_basic_node<BasicNodeType>,
                                      negation<std::is_same<typename BasicNodeType::string_type,
...
template <
    ...
    enable_if_t<
        conjunction<is_basic_node<BasicNodeType>,
...
template <typename BasicNodeType, typename NullType,
          enable_if_t<conjunction<is_basic_node<BasicNodeType>,
                                  std::is_same<NullType, std::nullptr_t>>::value,
                      int> = 0>
...
template <typename BasicNodeType, typename T,
          enable_if_t<conjunction<is_basic_node<BasicNodeType>,
                                  std::is_same<typename BasicNodeType::boolean_type, T>>::value,
                      int> = 0>
...
template <typename BasicNodeType, typename T,
          enable_if_t<conjunction<is_basic_node<BasicNodeType>, std::is_floating_point<T>>::value,
                      int> = 0>
...
template <
    typename BasicNodeType, typename T,
    enable_if_t<
        conjunction<is_basic_node<BasicNodeType>, negation<is_null_pointer<T>>,
...
struct to_node_fn
{
    ...
    /// @return decltype(to_node(n, std::forward<T>(val))) void by default. User can set it to some
    /// other type.
    template <typename BasicNodeType, typename T>
    auto operator()(BasicNodeType &n, T &&val) const
        noexcept(noexcept(to_node(n, std::forward<T>(val))))
...
/// @sa https://fktn-k.github.io/fkYAML/api/node_value_converter/
template <typename ValueType, typename> class node_value_converter
{
  public:
...
/// @tparam Allocator A class for allocators.
/// @sa https://fktn-k.github.io/fkYAML/api/ordered_map/
template <typename Key, typename Value, typename IgnoredCompare = std::less<Key>,
...
class basic_node
{
  public:
    ...
    template <typename T, typename SFINAE> using value_converter_type = ConverterType<T, SFINAE>;
  ...
  private:
    template <node_type> friend struct fkyaml::detail::external_node_constructor;

    template <typename BasicNodeType> friend class fkyaml::detail::basic_deserializer;

    template <typename BasicNodeType> friend class fkyaml::detail::basic_serializer;
    ...
    /// @details This union combines the different storage types for the YAML value types defined in
    /// @ref node_t.
    /// @note Container types are stored as pointers so that the size of this union will not exceed
    /// 64 bits by default.
    union node_value {
        /// @brief Constructs a new basic_node Value object for null types.
        node_value() = default;

        /// @brief Constructs a new basic_node value object with a node type. The default value for
        /// the specified type will be assigned.
        ...
        /// @brief Destroys the existing Node value. This process is recursive if the specified node
        /// type is for containers.
        ...
        /// A pointer to the value of mapping type. This pointer is also used when node type is
        /// null.
  ...
  private:
    ...
    static ObjType *create_object(ArgTypes &&...args)
    {
        ...
        std::unique_ptr<ObjType, decltype(deleter)> object(AllocTraitsType::allocate(alloc, 1),
                                                           deleter);
    ...
    template <typename ObjType> static void destroy_object(ObjType *obj)
    {
  ...
  public:
    ...
    explicit basic_node(const node_t type) : basic_node(detail::convert_to_node_type(type))
    {
    }
    ...
    basic_node(const basic_node &rhs)
        : m_attrs(rhs.m_attrs), mp_meta(rhs.mp_meta), m_prop(rhs.m_prop)
    {
        if FK_YAML_LIKELY (!has_anchor_name())
        {
            ...
            case detail::node_attr_bits::seq_bit:
                m_node_value.p_sequence =
                    create_object<sequence_type>(*(rhs.m_node_value.p_sequence));
                break;
            case detail::node_attr_bits::map_bit:
    ...
    basic_node(basic_node &&rhs) noexcept
        : m_attrs(rhs.m_attrs), mp_meta(std::move(rhs.mp_meta)), m_prop(std::move(rhs.m_prop))
    {
        if FK_YAML_LIKELY (!has_anchor_name())
        {
            switch (m_attrs & detail::node_attr_mask::value)
    ...
    template <typename CompatibleType, typename U = detail::remove_cvref_t<CompatibleType>,
              detail::enable_if_t<
                  ...
                  int> = 0>
    basic_node(CompatibleType &&val) noexcept(noexcept(ConverterType<U, void>::to_node(
        std::declval<basic_node &>(), std::declval<CompatibleType>())))
    {
    ...
    /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/constructor/
    template <typename NodeRefStorageType,
    ...
    {
        bool is_mapping = std::all_of(init.begin(), init.end(),
                                      [](const detail::node_ref_storage<basic_node> &node_ref) {
        ...
        if (is_mapping)
        {
            ...
            for (auto &elem_ref : init)
            {
                ...
                m_node_value.p_mapping->emplace(std::move((*(elem.m_node_value.p_sequence))[0]),
    ...
    ~basic_node() noexcept // NOLINT(bugprone-exception-escape)
    {
        if (m_attrs & detail::node_attr_mask::anchoring)
        {
            if (m_attrs & detail::node_attr_bits::anchor_bit)
            {
                ...
                itr->second.m_node_value.destroy(itr->second.m_attrs &
                                                 detail::node_attr_mask::value);
  ...
  public:
    ...
    template <typename InputType> static basic_node deserialize(InputType &&input)
    {
        return deserializer_type().deserialize(
            detail::input_adapter(std::forward<InputType>(input)));
    }

    /// @brief Deserialize the first YAML document in the input ranged by the iterators into a
    /// basic_node object.
    ...
    /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/deserialize/
    template <typename ItrType> static basic_node deserialize(ItrType begin, ItrType end)
    {
        return deserializer_type().deserialize(
    ...
    template <typename InputType> static std::vector<basic_node> deserialize_docs(InputType &&input)
    {
        return deserializer_type().deserialize_docs(
            detail::input_adapter(std::forward<InputType>(input)));
    }

    /// @brief Deserialize all YAML documents in the input ranged by the iterators into basic_node
    /// objects.
    /// @tparam ItrType Type of a compatible iterator.
    /// @param[in] begin An iterator to the first element of an input sequence.
    /// @param[in] end An iterator to the past-the-last element of an input sequence.
    ...
    /// @brief A factory method for alias basic_node objects referencing the given anchor basic_node
    /// object.
  ...
  public:
    ...
    /// @brief A subscript operator of the basic_node class with a key of a compatible type with
    /// basic_node.
    /// @tparam KeyType A key type compatible with basic_node
    /// @param key A key to the target value in a sequence/mapping node.
    /// @return The value associated with the given key, or a default basic_node object associated
    /// with the given key.
    /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/operator[]/
    template <typename KeyType,
              detail::enable_if_t<
                  detail::conjunction<detail::negation<detail::is_basic_node<KeyType>>,
    ...
    template <
        typename KeyType,
        detail::enable_if_t<detail::is_basic_node<detail::remove_cvref_t<KeyType>>::value, int> = 0>
    basic_node &operator[](KeyType &&key)
    {
        if FK_YAML_UNLIKELY (is_scalar())
        {
    ...
    bool operator==(const basic_node &rhs) const noexcept
    {
        ...
        case detail::node_attr_bits::int_bit:
            ...
            break;
        case detail::node_attr_bits::float_bit:
            ret = (std::abs(this_node_value_ptr->float_val - other_node_value_ptr->float_val) <
    ...
    bool operator<(const basic_node &rhs) const noexcept
    {
        ...
        const detail::node_attr_t this_val_bit = get_node_attrs() & detail::node_attr_mask::value;
        const detail::node_attr_t other_val_bit =
            rhs.get_node_attrs() & detail::node_attr_mask::value;
  ...
  public:
    ...
    template <typename KeyType,
              detail::enable_if_t<
                  detail::conjunction<
                      ...
                      detail::is_node_compatible_type<basic_node,
                                                      detail::remove_cvref_t<KeyType>>>::value,
                  int> = 0>
    ...
    basic_node &at(KeyType &&key)
    {
        ...
        if (is_sequence())
        {
            ...
            {
                throw fkyaml::type_error(
                    "An argument of at() for sequence nodes must be an integer.", get_type());
            }
    ...
    /// @note Some anchor name must be set before calling this method. Call has_anchor_name() to see
    /// if this basic_node object has any anchor name.
    ...
    /// @note If this basic_node object has already had any anchor name, the new anchor name will
    ...
    /// @note Some tag name must be set before calling this method. Call has_tag_name() to see if
    /// this basic_node object has any tag name.
    ...
    /// @note If this basic_node object has already had any tag name, the new tag name will
    /// overwrite the old one.
    ...
    /// @note This function requires T objects to be default constructible. Also, T cannot be either
    /// a reference, pointer or C-style array type.
    ...
    template <typename T, typename ValueType = detail::remove_cv_t<T>,
              detail::enable_if_t<std::is_default_constructible<ValueType>::value, int> = 0>
    T get_value() const
        noexcept(noexcept(std::declval<const basic_node &>().template get_value_impl<ValueType>(
            std::declval<ValueType &>())))
    {
        // emit a compile error if T is either a reference, pointer or C-style array type.
        static_assert(!std::is_reference<T>::value,
                      "get_value() cannot be called with reference types. you might want to call "
                      "get_value_ref().");
        static_assert(!std::is_pointer<T>::value,
                      "get_value() cannot be called with pointer types.");
        static_assert(!std::is_array<T>::value,
                      "get_value() cannot be called with C-style array types. you might want to "
                      "call get_value_inplace().");
    ...
    /// @brief Get the node value object converted into a given type. The conversion result is
    /// filled into `value_ref`.
    ...
    template <typename T>
    void get_value_inplace(T &value_ref) const noexcept(noexcept(
        std::declval<const basic_node &>().template get_value_impl<T>(std::declval<T &>())))
    {
        if (has_anchor_name())
        {
            auto itr = mp_meta->anchor_table.equal_range(m_prop.anchor).first;
    ...
    template <typename ReferenceType,
              detail::enable_if_t<std::is_reference<ReferenceType>::value, int> = 0>
    ReferenceType get_value_ref()
    {
        if (has_anchor_name())
        {
            auto itr = mp_meta->anchor_table.equal_range(m_prop.anchor).first;
            std::advance(itr, detail::node_attr_bits::get_anchor_offset(m_attrs));
            return itr->second.get_value_ref_impl(
                static_cast<detail::add_pointer_t<ReferenceType>>(nullptr));
        }
    ...
    template <
        ...
        detail::enable_if_t<
            detail::conjunction<std::is_reference<ReferenceType>,
                                std::is_const<detail::remove_reference_t<ReferenceType>>>::value,
            int> = 0>
    ...
    iterator begin()
    {
        ...
        default:
            throw fkyaml::type_error("The target node is neither of sequence nor mapping types.",
                                     get_type());
        }
    }

    /// @brief Returns a const iterator to the first element of a container node (sequence or
    /// mapping).
    /// @throw `type_error` if this basic_node is neither a sequence nor mapping node.
    ...
    /// @brief Returns an iterator to the past-the-last element of a container node (sequence or
    /// mapping).
    ...
    /// @brief Returns a const iterator to the past-the-last element of a container node (sequence
    /// or mapping).
    /// @throw `type_error` if this basic_node is neither a sequence nor mapping node.
    ...
    /// @brief Returns an iterator to the reverse-beginning (i.e., last) element of a container node
    /// (sequence or mapping).
    /// @throw `type_error` if this basic_node is neither a sequence nor mapping node.
    ...
    /// @brief Returns a const iterator to the reverse-beginning (i.e., last) element of a container
    /// node (sequence or mapping).
    /// @throw `type_error` if this basic_node is neither a sequence nor mapping node.
    ...
    /// @brief Returns an iterator to the reverse-end (i.e., one before the first) element of a
    /// container node (sequence or mapping).
    /// @throw `type_error` if this basic_node is neither a sequence nor mapping node.
    ...
    /// @brief Returns a const iterator to the reverse-end (i.e., one before the first) element of a
    /// container node (sequence or mapping).
    /// @throw `type_error` if this basic_node is neither a sequence nor mapping node.
  ...
  private:
    /// @brief Returns the pointer to the node_value object of either this node or the associated
    /// anchor node.
    /// @return The pointer to the node_value object of either this node or the associated anchor
    /// node.
    ...
    template <
        typename ValueType,
        detail::enable_if_t<detail::negation<detail::is_basic_node<ValueType>>::value, int> = 0>
    void get_value_impl(ValueType &v) const noexcept(
        noexcept(ConverterType<ValueType, void>::from_node(std::declval<const basic_node &>(), v)))
    {
        ConverterType<ValueType, void>::from_node(*this, v);
    }

    template <typename ValueType,
              detail::enable_if_t<detail::is_basic_node<ValueType>::value, int> = 0>
    ...
    mutable std::shared_ptr<detail::document_metainfo<basic_node>> mp_meta{
        ...
        std::shared_ptr<detail::document_metainfo<basic_node>>(
            new detail::document_metainfo<basic_node>())};
...
template <template <typename, typename...> class SequenceType,
          ...
          typename IntegerType, typename FloatNumberType, typename StringType,
          template <typename, typename = void> class ConverterType>
inline void swap(basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType,
                            StringType, ConverterType> &lhs,
                 basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType,
                            StringType, ConverterType> &rhs) noexcept(noexcept(lhs.swap(rhs)))
{
    lhs.swap(rhs);
}

/// @brief Insertion operator for basic_node template class. A wrapper for the serialization
/// feature.
...
inline std::ostream &operator<<(
    std::ostream &os, const basic_node<SequenceType, MappingType, BooleanType, IntegerType,
                                       FloatNumberType, StringType, ConverterType> &n)
{
    os << basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType,
                     StringType, ConverterType>::serialize(n);
    return os;
}

/// @brief Extraction operator for basic_node template class. A wrapper for the deserialization
/// feature with input streams.
...
inline std::istream &operator>>(std::istream &is,
                                basic_node<SequenceType, MappingType, BooleanType, IntegerType,
                                           FloatNumberType, StringType, ConverterType> &n)
{
    n = basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType, StringType,
                   ConverterType>::deserialize(is);
    return is;
}
...
// Whitespace before the literal operator identifier is deprecated in C++23 or better but required
// in C++11. Ignore the warning as a workaround. https://github.com/fktn-k/fkYAML/pull/417
...
/// @brief The user-defined string literal which deserializes a `char16_t` array into a `node`
/// object.
...
/// @brief The user-defined string literal which deserializes a `char32_t` array into a `node`
/// object.
...
#if FK_YAML_HAS_CHAR8_T
/// @brief The user-defined string literal which deserializes a `char8_t` array into a `node`
/// object.
...
struct hash<fkyaml::basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType,
                               StringType, ConverterType>>
{
    using node_t = fkyaml::basic_node<SequenceType, MappingType, BooleanType, IntegerType,
                                      FloatNumberType, StringType, ConverterType>;
    ...
    std::size_t operator()(const node_t &n) const
    {
        ...
        case fkyaml::node_type::FLOAT:
            hash_combine(seed,
                         std::hash<float_number_type>()(n.template get_value<float_number_type>()));
            return seed;
