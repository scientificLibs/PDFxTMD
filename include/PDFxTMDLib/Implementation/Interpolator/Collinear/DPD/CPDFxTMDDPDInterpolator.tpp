#include "CPDFxTMDDPDInterpolator.h"
...
const IReader<Reader> *CPDFxTMDDPDInterpolator<Reader>::getReader() const
{
    return m_reader;
}

template <class Reader>
void CPDFxTMDDPDInterpolator<Reader>::initialize(const IReader<Reader> *reader)
{
    m_reader = reader;
...
template <class Reader>
double CPDFxTMDDPDInterpolator<Reader>::interpolate(
    ...
    MU_T mu2_2) const
{
    ...
    if (mu1_2 <= 0.0 || mu2_2 <= 0.0)
        throw std::runtime_error("DPD interpolation requires positive mu^2 values");

    double t1 = std::log(mu1_2);
    double t2 = std::log(mu2_2);

    // Important:
    // DPDB1 stores the full requested scale square.
