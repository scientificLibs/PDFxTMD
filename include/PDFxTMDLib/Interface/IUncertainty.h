...
class IUncertainty
{
  public:
    ...
    explicit IUncertainty(UncertaintyApproachT uncertaintyApproach)
        : pimpl_(new OwningModel<UncertaintyApproachT>(std::move(uncertaintyApproach)),
          ...
          uncertaintyOperation_(
              [](void *uncertaintyApproachBytes, const std::vector<double> &values, const int numCoreErrMember, const double cl, PDFUncertainty& uncertainty) -> void {
                  using Model = OwningModel<UncertaintyApproachT>;
                  auto *const model = static_cast<Model *>(uncertaintyApproachBytes);
                  return model->Uncertainty(values, numCoreErrMember, cl, uncertainty); 
              }),
              correlationOperation_(
              [](void *uncertaintyApproachBytes, const std::vector<double>& valuesA, const std::vector<double>& valuesB, const int numCoreErrMember) -> double{
                  using Model = OwningModel<UncertaintyApproachT>;
                  auto *const model = static_cast<Model *>(uncertaintyApproachBytes);
    ...
    IUncertainty(const IUncertainty &other)
        : pimpl_(other.clone_(other.pimpl_.get()), other.pimpl_.get_deleter()),
          clone_(other.clone_),
        correlationOperation_(other.correlationOperation_),
  ...
  private:
    ...
    {
        OwningModel(UncertaintyApproachT uncertaintyApporach) : uncertaintyApporach_(std::move(uncertaintyApporach))
        {
        }
    ...
    using UncertaintyOperation = void(void *, const std::vector<double> &, const int, const double, PDFUncertainty&);
    using CorrelationOperation = double (void *, const std::vector<double>&, const std::vector<double>&, const int);
