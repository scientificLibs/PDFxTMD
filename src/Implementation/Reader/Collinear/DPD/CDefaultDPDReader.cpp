...
void validateMagic(const std::array<char, 16> &magic)
{
    if (!(magic[0] == 'P' && magic[1] == 'D' && magic[2] == 'F' &&
          ...
          magic[11] == 'B' && magic[12] == '1'))
    {
        throw std::runtime_error("not a PDFxTMD-DPDB1 file");
    }
}

} // namespace

void CDefaultDPDReader::read(const std::string &pdfName, int setNumber)
{
    ...
    if (version != 1)
        throw std::runtime_error("unsupported PDFxTMD-DPDB1 version");

    if (npartons != UnEqualScaleGridData::kNPartons)
        throw std::runtime_error("unexpected npartons in PDFxTMD-DPDB1");
    ...
    for (uint32_t p = 0; p < nphys; ++p)
    {
        ...
        if (m_gridData.phys_pairs[p].i >= nx || m_gridData.phys_pairs[p].j >= nx)
            throw std::runtime_error("pair index out of range in DPDB1");
    }
...
UnEqualScaleGridData CDefaultDPDReader::getData() const
{
    return m_gridData;
}

std::vector<double> CDefaultDPDReader::getValues(PhaseSpaceComponent comp) const
{
...
std::pair<double, double> CDefaultDPDReader::getBoundaryValues(
